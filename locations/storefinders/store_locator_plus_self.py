...
class StoreLocatorPlusSelfSpider(Spider):
    """
    This store finder is a self-hosted WordPress plugin with a website of
    https://wordpress.org/plugins/store-locator-le/ and source code at
    https://github.com/wp-plugins/store-locator-le

    This store finder is not to be confused with the software-as-a-service
    store locator of the same name, from the same company, that is documented
    at https://storelocatorplus.com/

    To use this spider, specify a value for allowed_domains and the default
    path for the StoreLocatorPlus API endpoint will be used. In the event the
    default path is different, you can alternatively specify a value for
    start_url. Then specify either:

    PREFERRED
    1. A list of ISO-3166 alpha-2 country codes as the iseadgg_countries_list
       parameter and a suitable non-zero search_radius value in kilometres.
       ...
       selected as the most appropriate to use against a 100km search radius
       accepted by the API endpoint.

    NONPREFERRED
    2. A list of searchable_points_files = [x, y..] suitable for use with the
       point_locations function of locations.geo and a suitable non-zero
       search_radius value in kilometres.

    A non-zero value is also required for the max_results attribute. This
    number is set by the server and cannot be changed. To obtain this
    max_results value, observe an API call and check the JSON response:
      data_queries["standard_location_load"]["query"]
    Then obtain the max_results number from the SQL LIMIT clause.
    ...
    been truncated. If this occurs, search_radius needs to be reduced to
    ensure that max_results (or more) locations are never returned for any
    radius search.
