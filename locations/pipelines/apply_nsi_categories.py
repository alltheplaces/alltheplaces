...
class ApplyNSICategoriesPipeline:
    ...
    def process_item(self, item: Feature):
        ...
        if len(matches) == 0 and item.get("brand_wikidata"):
            self.crawler.stats.inc_value("atp/nsi/brand_missing")  # ty: ignore[unresolved-attribute]
            return item
        elif len(matches) == 0 and item.get("operator_wikidata"):
            self.crawler.stats.inc_value("atp/nsi/operator_missing")  # ty: ignore[unresolved-attribute]
            return item

        if len(matches) == 1:
            self.crawler.stats.inc_value("atp/nsi/perfect_match")  # ty: ignore[unresolved-attribute]
        ...
        if cc := item.get("country"):
            ...
            if len(matches) == 1:
                self.crawler.stats.inc_value("atp/nsi/cc_match")  # ty: ignore[unresolved-attribute]
                return self.apply_tags(matches[0], item)
        ...
        if categories := get_category_tags(item):
            ...
            if len(matches) == 1:
                self.crawler.stats.inc_value("atp/nsi/category_match")  # ty: ignore[unresolved-attribute]
                return self.apply_tags(matches[0], item)

        self.crawler.stats.inc_value("atp/nsi/match_failed")  # ty: ignore[unresolved-attribute]
        return item
