...
async def click_solver(page: Page, request: Request, spider: Spider) -> None:
    """
    For a Playwright Page object returned from a Camoufox spider, detect if
    there is a click-solver captcha present to solve, then solve it
    automatically. See locations/middlewares/playwright_middleware.py for more
    description of what a Camoufox spider is.

    Click-solver captchas are solved by the user clicking on an object on the
    page. Implementations include:
    ...
    if await page.title() == "Just a moment...":
        ...
        # generate a noisy exception in doing so. This noise can be avoided
        # by waiting a few seconds to start the captcha solver.
