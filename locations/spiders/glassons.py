...
class GlassonsSpider(Spider):
    ...
    def parse(self, response):
        location_data = response.xpath('//script[contains(text(), "var ga_stores = ")]/text()').get().split("var ga_stores = ", 1)[1].split(" // all store data", 1)[0]
        ...
        for location_ref, location in locations.items():
            ...
            item["website"] = "https://www.glassons.com/store-locations/" + location["region"].lower() + "/" + location["label"].lower().replace(",", "").replace(" ", "-")
            item["opening_hours"] = OpeningHours()
            item["opening_hours"].add_ranges_from_string(re.sub(r"\s+", " ", location["openinghours"].replace(",", " ")))
            yield item
