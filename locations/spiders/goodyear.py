...
class GoodYearSpider(scrapy.Spider):
    ...
    start_urls = (
        'https://www.goodyear.com/en-US/tires/tire-shop',
    )
    ...
    def parse_state(self, response):
        stateurls = response.xpath('//div[@class="sub-group col-lg-4"]//a/@href').extract()
    ...
    def parse_city(self, response):
        ...
        for cityurl in cityurls:
            yield scrapy.Request(response.urljoin(cityurl), callback=self.parse_location)

    def parse_location(self, response):
        store_js = response.xpath('//script[@type="text/javascript" and contains(text(), "formattedAddress")]/text()').extract()
