...
class BiggbyCoffeeUSSpider(Spider):
    ...
    def parse(self, response):
        for location in response.xpath('(//markers)[1]/marker'):
            ...
            item["street_address"] = ", ".join(filter(None, [location.attrib["address-one"], location.attrib["address-two"]]))
            item["opening_hours"] = OpeningHours()
            item["opening_hours"].add_days_range(["Mo", "Tu", "We", "Th"], location.attrib["mon-thurs-open-hour"], location.attrib["mon-thurs-open-hour"])
