...
class HarrisFarmMarketsAUSpider(CrawlSpider):
    ...
    rules = [Rule(LinkExtractor(allow=r"^https:\/\/www\.harrisfarm\.com\.au\/pages\/[\w\-]+$", deny=r"-butcher$", restrict_xpaths='//div[contains(@class, "HF-new-blockstyle-card-UI")]'), callback="parse")]

    def parse(self, response):
        properties = {
            "ref": response.url,
            "name": response.xpath('//div[@class="child"]/h1[@class="title"]/a/text()').get(),
            "addr_full": re.sub(r"\s+", " ", ", ".join(filter(None, response.xpath('//div[@id="contacts"]/p//text()').getall()))),
            "phone": response.xpath('//text()[contains(., "Telephone:")]/ancestor::*[self::strong][1]/ancestor::*[self::li][1]/text()').get(),
            "email": "".join(filter(None, response.xpath('//text()[contains(., "Email:")]/ancestor::*[self::strong][1]/ancestor::*[self::li][1]/a//text()').getall())),
        ...
        if latlng:
            properties["lat"], properties["lon"] = latlng.split("google.maps.LatLng(", 1)[1].split(")", 1)[0].split(",", 1)
