...
class SdekSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for poi in data.get('data', {}).get('data', []):
            yield JsonRequest('https://www.cdek.ru/api-site/v1/graphql/', data=self.graphql_query(poi['id']), callback=self.parse_poi)
    ...
    def parse_hours(self, item, poi):
        if hours := poi.get('worktimes'):
            try:
                oh = OpeningHours()
                for hour in hours:
                    oh.add_range(day=DAYS[hour['day'] - 1], open_time=hour['startTime'], close_time=hour['stopTime'], time_format='%H:%M:%S')
    ...
    def graphql_query(self, poi_id):
        return {"query" : '''query websiteOffice(
            $id: UID!
            ) {
            websiteOffice(id: $id)
            {
