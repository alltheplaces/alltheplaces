...
class KFCCanadaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        store_details = response.xpath('//script[contains(text(), "allRestDetails")]/text()').extract_first()
        data = json.loads(re.search(r'var allRestDetails = \'(.*)\';', store_details).group(1))
        for store in data:
