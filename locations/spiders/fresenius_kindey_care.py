...
class FreseniusFidneyCare(scrapy.Spider):
    ...
    def parse_hours(self, hours):
        ...
        for group in hours:
            ...
            days = days.split(',')
            for day in days:
                opening_hours.add_range(day=DAY_MAPPING[day], open_time=open_time, close_time=close_time, time_format='%H:%M')

        return opening_hours.as_opening_hours()

    def parse(self, response):
        try:
            extracted_script = response.xpath('//script[@type="application/ld+json" and contains(text(), "streetAddress")]/text()').extract_first()
