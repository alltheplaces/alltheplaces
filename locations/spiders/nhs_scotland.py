...
class NHSScotlandSpider(scrapy.Spider):
    ...
    def parse(self, response):
        urls = response.xpath('//div[@class="js-equal-height blockgrid-list"]/a/@href').extract()

        for url in urls:
            yield scrapy.Request(response.urljoin(url), callback=self.parse_directory)

    def parse_directory(self, response):
        directory_urls = response.xpath('//ul[@class="search__results"]/li/div/h3/a/@href').extract()
    ...
    def parse_location(self, response):
        ...
        try:
            geo = response.xpath('//div[@class="cf bg-light-grey-1 soft-half"]/a/@href').extract_first()
