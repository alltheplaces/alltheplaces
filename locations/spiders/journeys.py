...
class JourneysSpider(scrapy.Spider):
    ...
    def parse_store(self, response):
        ...
        name = response.xpath('normalize-space(//p[@itemprop="streetAddress"]//text())').extract_first()  # usually the venue name
        address = response.xpath('normalize-space(//div[@itemprop="address"]//p[2]/text())').extract_first()
        if not address:
    ...
    def parse(self, response):
        states = response.xpath('//select[@name="StateOrProvince"]/option/@value').extract()
        for state in states:
            if state:
                form_data = {
                ...
                yield scrapy.http.FormRequest(url='https://www.journeys.com/stores',
                                              method='POST',
                                              formdata=form_data,
                                              # headers=HEADERS,
