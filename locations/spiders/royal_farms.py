...
class Royal_farmsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        states = response.xpath(
            '//select[@id="state"]/option/@value').extract()
    ...
    def parse_stores(self, response):
        ...
        for row in rows:
            ...
            amenities = row.xpath(
                './td[@class="listdata"][3]/img/@src').getall()

            store_number = store.xpath(
                './strong/text()').re_first(r'STORE #(\d+)')
