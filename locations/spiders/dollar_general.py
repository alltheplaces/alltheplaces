...
class DollarGeneralSpider(scrapy.Spider):
    ...
    def start_requests(self):
        url = (
            "https://hosted.where2getit.com/dollargeneral/rest/locatorsearch?lang=fr_FR"
        )
    ...
    def parse(self, response):
        ...
        if "collection" in result["response"].keys():
            for data in result["response"]["collection"]:
                ...
                postalcode = (
                    "postalcode" in data.keys()
                    and re.search(r"\d+", data["postalcode"]).group()
                    or ""
                )
