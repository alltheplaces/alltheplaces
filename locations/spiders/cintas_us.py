    ...
    def parse(self, response):
        for location in response.xpath('//li[@data-location]'):
            properties = {
                ...
                "addr_full": re.sub(r"\s+", " ", ", ".join(filter(None, location.xpath('.//p[@class="locations-results__data-address"]/text()').getall()))).strip(),
            }
            if phone := location.xpath('.//div[@class="locations-results__data-tel"]/p[2]/a/@href').get():
                properties["phone"] = phone
            properties["opening_hours"] = OpeningHours()
            properties["opening_hours"].add_ranges_from_string(" ".join(filter(None, location.xpath('.//div[@class="locations-results__data-tel"]/p[1]/text()').getall()))),
