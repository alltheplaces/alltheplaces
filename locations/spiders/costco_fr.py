...
from locations.hours import OpeningHours, CLOSED_FR, DAYS_FR, DELIMITERS_FR
...
class CostcoFRSpider(CostcoAUSpider):
    ...
    def parse_hours_string(self, hours_string: str) -> OpeningHours:
        hours_string = re.sub(r"(?<=\d)h(?!\d)", ":00", re.sub(r"(?<=\d)h(?=\d)", ":", hours_string.replace(".:", ":").replace("station service", "").replace("Tous les dimanches", "Dimanche").replace("00:00 - 00:00", "Fermée")))
