    ...
    def start_requests(self):
        for url in self.start_urls:
            if "GetStoresByState" in url:
                ...
                yield JsonRequest(url=url, method="POST", data={"countryCode": "NZ", "latitude": -36.85, "longitude": 174.76}) # Auckland
                yield JsonRequest(url=url, method="POST", data={"countryCode": "NZ", "latitude": -41.29, "longitude": 174.79}) # Wellington
                yield JsonRequest(url=url, method="POST", data={"countryCode": "NZ", "latitude": -43.53, "longitude": 172.63}) # Christchurch

    def parse(self, response):
        cleaned_json = response.text[1:-1].replace('\\"', '"').replace('\\\\r', '').replace('\\\\n', ' ').replace('\\\\/', '')
