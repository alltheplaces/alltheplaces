    ...
    def parse(self, response):
        for location in response.xpath('//div[@class="store-item container" and @data-store-id]'):
            properties = {
                ...
                "addr_full": unescape(re.sub(r"\s+", " ", "".join(filter(None, location.xpath('.//div[contains(@class, "store-address")]/text()').getall())))).strip(),
            ...
            if "CLOSED" in properties["name"].upper().split():
                continue
            if m := re.search(r"https:\/\/maps\.google\.com\/\?daddr=(-?\d+\.\d+),(-?\d+\.\d+)", location.xpath('.//a[@class="store-map"]/@href').get()):
            ...
            hours_string = re.sub(r"\s+", " ", "".join(filter(None, location.xpath('.//div[contains(@class, "store-hours")]//text()').getall()))).strip()
