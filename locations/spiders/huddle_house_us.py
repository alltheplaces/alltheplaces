...
class HuddleHouseUSSpider(Spider):
    ...
    def start_requests(self):
        for url in self.start_urls:
            for lat, lon in point_locations("us_centroids_iseadgg_48km_radius.csv"):
                yield JsonRequest(url=f"{url}?lat={lat}&lng={lon}&radius=50", headers={"path": "locations/search", "client_type": "web"})
    ...
    def parse(self, response):
        ...
        for location in response.json()["locations"]:
            ...
            apply_yes_no(PaymentMethods.AMERICAN_EXPRESS, item, "American Express" in location["payments"]["cardtypes"], False)
