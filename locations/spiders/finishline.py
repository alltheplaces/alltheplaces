...
class FinishlineSpider(scrapy.Spider):
    ...
    def parse_stores(self, response):
        ...
        properties = {
            ...
            "brand": response.xpath('//span[@class="location-name-brand"]/text()')
            .extract_first()
            .strip(),
        }
    ...
    def parse_city_stores(self, response):
        ...
        for store in stores:
            ...
            if url:
                ...
                address_parts = store.xpath(
                    './/span[@class="c-address-street"]/span/text()'
                ).extract()
                brand = (
                ...
                hours = self.parse_hours(
                    store.xpath('//tr[@itemprop="openingHours"]/@content').extract()
                )
                if hours:
                    properties["opening_hours"] = hours
                yield GeojsonPointItem(**properties)
