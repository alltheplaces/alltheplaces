...
class BargainBoozeGBSpider(SitemapSpider, StructuredDataSpider):
    ...
    def post_process_item(self, item, response, ld_data):
        ...
        item["name"] = re.sub(r"\s*(?:Bargain Booze(?: Plus)?|-?\s*Under New Management|-?\s*Sole Trader|-?\s*Now Open|\.{2,})\s*", " ", item["name"], flags=re.IGNORECASE).strip()
        if "SELECT CONVENIENCE" in item["name"].upper() or "SELECT CONVENINECE" in item["name"].upper():
            ...
            item["name"] = re.sub(r"\s*(?:Select Convenience|Select Conveninece)\s*", " ", item["name"], flags=re.IGNORECASE).strip()
        ...
        elif "INSIDE FOOD WAREHOUSE" in item["name"].upper() or "THE FOOD WAREHOUSE" in item["name"].upper() or "FOOD WAREHOUSE" in item["name"].upper():
            ...
            item["name"] = re.sub(r"\s*(?:Inside Food Warehouse|The Food Warehouse|Food Warehouse)\s*", " ", item["name"], flags=re.IGNORECASE).strip()
        elif "INSIDE A COSTCUTTER" in item["name"].upper() or "INSIDE COSTCUTTER" in item["name"].upper() or "COSTCUTTER" in item["name"].upper():
            ...
            item["name"] = re.sub(r"\s*(?:Inside a Costcutter|Inside Costcutter|Costcutter)\s*", " ", item["name"], flags=re.IGNORECASE).strip()
        item.pop("facebook")
        item.pop("twitter")
        item.pop("image")
        yield item
