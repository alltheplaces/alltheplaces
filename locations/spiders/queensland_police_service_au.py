    ...
    def parse(self, response):
        ...
        for location2 in locations2:
            properties2 = {
                ...
                "addr_full": " ".join(
                    filter(None, location2.xpath('.//div[@class="row"]/div[@class="cols big"]//text()').getall())
                ).strip(),
                "phone": location2.xpath('.//a[contains(@href, "tel:")]/@href').get().replace("tel:", "").strip(),
            }
            hours_string = " ".join(
                filter(None, location2.xpath('.//div[@class="row"]/div[@class="cols smaller"][1]//text()').getall())
            ).strip()
