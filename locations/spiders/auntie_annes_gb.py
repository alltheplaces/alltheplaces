...
from scrapy.http import JsonRequest, Request, TextResponse, Response
...
class AuntieAnnesGBSpider(JSONBlobSpider):
    ...
    def extract_json(self, response: Response) -> dict | list[dict]:
        ...
        script = response.xpath(f"//script[contains(text(), {search!r})]/text()").get().split('"data_objects":')[1].split(',"schema":')[0] + "}"
