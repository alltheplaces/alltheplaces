...
class ChickenTreatAUSpider(Spider):
    ...
    start_urls = ["https://www.chickentreat.com.au/api-proxy/stores?include=amenities,availability,delivery,collection,holiday,storeAddress,salesforce"]
    ...
    def parse(self, response):
        for location in response.json()["data"]:
            ...
            item["website"] = "https://www.chickentreat.com.au/locations/" + location["attributes"]["storeName"].lower().replace(" ", "-")
            ...
            for day_hours in location["relationships"]["collection"]["data"]["attributes"]["collectionTimes"]:
                for time_period in day_hours["collectionTimePeriods"]:
                    item["opening_hours"].add_range(day_hours["dayOfWeek"], time_period["openTime"], time_period["closeTime"], "%H:%M:%S")
            apply_yes_no(Extras.TOILETS, item, location["relationships"]["amenities"]["data"]["attributes"]["haveToilet"], False)
            apply_yes_no(Extras.WIFI, item, location["relationships"]["amenities"]["data"]["attributes"]["haveWifi"], False)
