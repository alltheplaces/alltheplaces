...
class AsdaSpider(scrapy.Spider):
    ...
    start_urls = (
#        'https://storelocator.asda.com/#!/search/index?qs=manchester&dym=0&page=2',
    ...
    def time_str(self,time):
        ...
        match=re.search(r'(\d{1,2})(pm|am|mp)\s*-\s*(\d{1,2})(pm|am|mp)',time.replace('Midnight','12pm'))
        if not match:
            return ''
    ...
    def parse(self, response):
        ...
        for place in shops:
            yield scrapy.Request('https://storelocator.asda.com/store/'+place['url_key'], callback=self.parse_shop, meta={
        ...
        if len(shops)==250:
            yield scrapy.Request(self.start_urls[0]+str(start), callback=self.parse,meta={'start':start+250})
