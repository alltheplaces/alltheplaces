    ...
    def parse(self, response: Response) -> Iterable[Feature]:
        for feature in response.xpath('//div[contains(@class, "stores-grid--store")]'):
            properties = {
                ...
                "branch": feature.xpath('./div[contains(@class, "store-details-wrapper")]/a/text()')
                .get("")
                .strip()
                .removeprefix("Goldmark AU ")
                .removeprefix("Goldmark NZ ")
                .split("(", 1)[0],
                "addr_full": clean_address(
                    feature.xpath('./div[contains(@class, "store-details-wrapper")]/p[last()]/text()').getall()
                ).removesuffix("Phone:"),
                "phone": feature.xpath(
                    './div[contains(@class, "store-details-wrapper")]/p[last()]/a[contains(@href, "tel:")]/@href'
                )
                .get()
                .removeprefix("tel:"),
            ...
            properties["lat"], properties["lon"] = url_to_coords(
                feature.xpath('./div[contains(@class, "store-directions-wrapper")]/a/@href').get()
            )

            hours_text = " ".join(
                filter(
                    None,
                    map(str.strip, feature.xpath('./div[contains(@class, "store-hours-wrapper")]//text()').getall()),
                )
            )
