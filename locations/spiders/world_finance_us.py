    ...
    def parse(self, response: Response) -> Any:
        for location in response.xpath('//div[@class="branch-data"]'):
            properties = {
                ...
                "street_address": clean_address([location.xpath('.//@data-branch-line1').get(), location.xpath('.//@data-branch-line2').get()]),
            ...
            yield Request(url=properties["website"], callback=self.parse_opening_hours, meta={"item": Feature(**properties)})

    def parse_opening_hours(self, response: Response) -> Any:
        item = response.meta["item"]
        hours_string = " ".join(filter(None, map(str.strip, response.xpath('//div[@class="dates_list"]//text()').getall())))
