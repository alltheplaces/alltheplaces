...
class NandosNZSpider(scrapy.Spider):
    name = "nandos_nz"
    item_attributes = {'brand': "Nando's", 'brand_wikidata': "Q3472954"}
    allowed_domains = ['www.nandos.co.nz']
    start_urls = [
    ...
    def parse(self, response):
        ...
        urls = response.xpath('//url/loc[contains(text(),"nz/restaurant/")]/text()').extract()

        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse_store)

    def parse_store(self, response):
        data = response.xpath('//script[@type="application/json" and contains(text(), "address")]/text()').extract_first()
