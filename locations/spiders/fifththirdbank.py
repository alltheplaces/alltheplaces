...
class FifthThirdBankSpider(scrapy.Spider):
    ...
    start_urls = (
        'https://locations.53.com/index.html',
    )
    ...
    def parse_branch(self, response):
        ...
        name = response.xpath('normalize-space(//h1[@class="c-location-title"]/span[@class="name"]/text())').extract_first()
        branch = response.xpath('normalize-space(//h1[@class="c-location-title"]/span[@class="geomodifier"]/text())').extract_first()
    ...
    def parse(self, response):
        branch_urls = response.xpath('//div[@class="location"]//a[@class="location-name-link"]/@href').extract()
        if branch_urls:
            for url in branch_urls:
                yield scrapy.Request(response.urljoin(url), callback=self.parse_branch)
                
        else:
            urls = response.xpath('//div[@class="c-directory-list"]//li/a/@href').extract()
