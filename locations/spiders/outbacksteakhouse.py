...
class OutbackSteakhouseSpider(scrapy.Spider):
    ...
    item_attributes = { 'brand': "Outback Steakhouse" }
    ...
    def parse(self, response):
        ...
        for stores_list in store_data:
            ...
            for store in stores:
                ...
                lat = latlong_data[(STATES[state]).upper()]["longLats"][storeid].get("lat")
                lon = latlong_data[(STATES[state]).upper()]["longLats"][storeid].get("long")
