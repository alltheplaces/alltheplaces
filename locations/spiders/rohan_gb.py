...
class RohanGBSpider(Spider):
    ...
    def parse(self, response: Response, **kwargs: Any) -> Any:
        content = (
            response.xpath('//script[text()[contains(., "pageContext")]]/text()')
            .get()
            .replace('const pageContext = JSON.parse("', "")
            .replace('");', "")
            .replace("const jsStores = pageContext['jsStores'];", "")
            .replace('\\"', '"')
        )
        ...
        for location in data["jsStores"]:
            ...
            item["street_address"] = merge_address_lines(
                [location["node"]["address"]["address1"], location["node"]["address"]["address2"]]
            )

            slug = location["node"]["label"]
