...
class CarrefourROSpider(Spider):
    ...
    brands = {
        ...
        "Express portocaliu": {"brand": "Carrefour Express", "brand_wikidata": "Q2940190"}, # Express - "Orange" store (?)
    ...
    def parse(self, response):
        js_dict = response.xpath('//script[contains(text(), "app.stores = app.allStores = ")]/text()').get().split("app.stores = app.allStores = ", 1)[1].split("}];", 1)[0] + "}]"
        for location in chompjs.parse_js_object(js_dict):
            item = DictParser.parse(location)
            if not location["status"]:
                continue
            if not location.get("type") or not location["type"].get("name") or location["type"]["name"] not in self.brands.keys():
            ...
            item["website"] = "https://carrefour.ro/corporate/magazine/" + location["city"]["slug"] + "/" + location["slug"]
