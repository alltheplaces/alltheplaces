...
class EvyveGBSpider(Spider):
    ...
    def parse(self, response):
        locations = json.loads(response.xpath('//main[@id="content"]/script').get().split("var propertyLocations = JSON.parse('", 2)[1].split("');", 2)[0])
        for location in locations:
            ...
            item["addr_full"] = ", ".join(filter(None, [location["address"], location["address_line_2"], location["city"]]))
            item.pop("city")
