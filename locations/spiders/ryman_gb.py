            ...
            if "locations" in script:
                ...
                for store in DictParser.get_nested_key(result, "locations"):
                    ...
                    for day in DAYS_FULL:
                        if "lunch_break_ends" in times[day.lower()] and times[day.lower()]["lunch_break_starts"] is not None:
                            opening_hours.add_range(
                                day=day, open_time=times[day.lower()]["open_time"], close_time=times[day.lower()]["lunch_break_starts"], time_format="%H:%M"
                            )
                            opening_hours.add_range(
                                day=day, open_time=times[day.lower()]["lunch_break_ends"], close_time=times[day.lower()]["closing_time"], time_format="%H:%M"
                            )
                        else:
                            opening_hours.add_range(
                                day=day, open_time=times[day.lower()]["open_time"], close_time=times[day.lower()]["closing_time"], time_format="%H:%M"
                            )
