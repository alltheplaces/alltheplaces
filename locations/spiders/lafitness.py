...
class LAFitnessSpider(scrapy.Spider):
    name = "lafitness"
    chain_name = "LA Fitness"
    allowed_domains = ["lafitness.com"]
    download_delay = 0.1

    def start_requests(self):
        data = {"zipCode": None, "state": None}
        yield scrapy.Request(url='https://www.lafitness.com/Pages/GetClubLocations.aspx/GetClubLocationsByStateAndZipCode',
                             method='POST',
    ...
    def parse_hours(self, elems):
        ...
        for hour in hours:
            ...
            if day_range:
                ...
                opening_hours.append("{}-{} {}-{}".format(DAY_MAPPING[day_start],
                                                          DAY_MAPPING[day_end],
                                                          open_time,
                                                          close_time))
            else:
                opening_hours.append("{} {}-{}".format(DAY_MAPPING[hour[0]],
                                                       open_time,
                                                       close_time))
    ...
    def parse(self, response):
        ...
        for location in locations:
            ...
            yield scrapy.Request('https://www.lafitness.com/pages/' + location["ClubHomeURL"],
