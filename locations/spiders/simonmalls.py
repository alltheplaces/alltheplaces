    ...
    def parse(self, response):
        ...
        for stores in jsonresponse:
            ...
            if store_data["address"]["country"] != "UNITED STATES":
                ...
                yield scrapy.Request(store_url, callback=self.parse_store, meta={"country": store_data["address"]["country"],"phone": store_data["mallPhone"],"location_type": store_data["propertyType"]})

    def parse_store(self,response):
        extracted_script = response.xpath('//script[@type="text/javascript" and contains(text(), "FullAddress")]/text()').extract_first()
        if extracted_script is None:
            ...
            properties = {
                ...
                'extras':
                    {
                        'location_type': response.meta["location_type"]
                    }
            }
