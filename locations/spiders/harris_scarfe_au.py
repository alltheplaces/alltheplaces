...
class HarrisScarfeAUSpider(SitemapSpider):
    ...
    def parse(self, response):
        properties = {
                ...
                "addr_full": re.sub(r"\s{2,}", " ", unicodedata.normalize("NFKD", html.unescape(" ".join(response.xpath('//div[contains(@class, "store-detail-desc")]/ul[1]/li/text()').getall()))).replace("\n", " ").strip()),
                "phone": response.xpath('//a[contains(@href, "tel:")]/text()').get().strip(),
                "website": response.url,
        }
        hours_string = " ".join(filter(None, response.xpath('//div[contains(@class, "store-detail")]/div[2]//table//td/text()').getall()))
