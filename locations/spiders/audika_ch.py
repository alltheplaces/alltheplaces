...
class AudikaCHSpider(SitemapSpider, StructuredDataSpider):
    name = "audika_ch"
    item_attributes = {"brand": "Audika", "brand_wikidata": "Q2870745"}
    sitemap_urls = ["https://www.audika.ch/sitemap.xml"]
    sitemap_rules = [(r"^https://www\.audika\.ch/de/hoercenter-finden/hoergeraete/audika-[^/]+$", "parse_sd")]
    search_for_facebook = False
    search_for_email = False

    def post_process_item(self, item: Feature, response: TextResponse, raw_data: dict, **kwargs) -> Iterable[Feature]:
        item["addr_full"] = item.pop("street_address")
        item["branch"] = html.unescape(item.pop("name")).removeprefix("Audika Hörcenter ")
