...
class AthletaSpider(scrapy.Spider):
    ...
    def store_hours(self, store_hours):
        ...
        for line in store_hours:
            if 'closed' in line:
                ...
                match = re.search(r'^([A-z]{1,2}) (\d{1,2})[:]?(\d{1,2})-(\d{1,2})[:]?(\d{1,2})$',
                                  line)
    ...
    def parse(self, response):
        data = response.xpath('//div[@class="cities"]/div[@class="subLocation"]/a/@href').extract()
        for store in data:
    ...
    def parse_store(self, response):
        ...
        name = store_data.xpath('.//div[@class="entry-content"]/h1/text()').extract()[0].split(' in ', 1)[0]
        store_details = store_data.xpath('.//div[@id="widget_Athleta_Stores_Details_div"]')
        ...
        properties = {
            ...
            "name": name,
            "opening_hours": self.store_hours(store_details.xpath('.//time[@itemprop="openingHours"]/@datetime')
                                              .extract()),
            "addr_full": addr1,
            "housenumber": num,
            "street": street,
