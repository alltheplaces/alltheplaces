...
from locations.hours import OpeningHours, DAYS_IT, CLOSED_IT, NAMED_DAY_RANGES_IT, NAMED_TIMES_IT
...
class CoopAlleanzaITSpider(JSONBlobSpider):
    ...
    def pre_process_data(self, location):
        ...
        location['website'] = f"https://www.coopalleanza3-0.it/fare-spesa/elenco-negozi/dettaglio-negozio/{location['detailsPage']}.html"
    ...
    def parse_store_extra(self, response, item):
        ...
        for day, hour in zip(hours[0::2], hours[1::2]):
            oh.add_ranges_from_string(f'{day} {hour}', days=DAYS_IT, named_day_ranges=NAMED_DAY_RANGES_IT, named_times=NAMED_TIMES_IT, closed_strings=CLOSED_IT)
        if oh:
    ...
    def apply_payment_methods(self, response, item):
        available_methods = clean_strings(response.xpath('//*[@data-component="relationStorePaymentCardsComponent"]//p/text()').getall())
    ...
    def apply_departments(self, response, item):
        departments = clean_strings(response.xpath('//*[@data-component="relationStoreDepartmentComponent"]//p/text()').getall())
