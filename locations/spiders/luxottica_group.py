...
class LuxotticaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if len(urls) == 0:
            properties = {
                'name': response.xpath(
                    '//div[@class="location-subheader"]/text() | //*[@class="c-location-title-geo h1-font-gray-5"]/text()').extract_first(),
                'addr_full': response.xpath(
                    '//*[@itemprop="streetAddress"]/text() | //*[@itemprop="streetAddress"]/@content').extract_first(),
                'city': response.xpath(
                    '//*[@itemprop="addressLocality"]/text() | //*[@itemprop="addressLocality"]/@content').extract_first(),
