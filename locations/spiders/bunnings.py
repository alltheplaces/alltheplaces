...
class BunningsSpider(Spider):
    ...
    def parse_idsrv_cookie_1(self, response: Response) -> Iterable[Request]:
        ...
        yield Request(url="https://authorisation.api.bunnings.com.au/External/Callback", meta={"dont_redirect": True, "handle_httpstatus_list": [302], "apigee_client_id": apigee_client_id, "nonce": nonce}, callback=self.parse_idsrv_cookie_2)

    def parse_idsrv_cookie_2(self, response: Response) -> Iterable[Request]:
        apigee_client_id = response.meta["apigee_client_id"]
        nonce = response.meta["nonce"]
        yield Request(url=f"https://authorisation.api.bunnings.com.au/connect/authorize/callback?response_type=token&scope=chk%3Aexec%20cm%3Aaccess%20ecom%3Aaccess%20chk%3Apub%20vch%3Apublic%20&client_id=budp_guest_user_au&redirect_uri=https%3A%2F%2Fwww.bunnings.com.au%2Fstatic%2Fguest.html&nonce={nonce}&acr_values=", meta={"dont_redirect": True, "handle_httpstatus_list": [302], "apigee_client_id": apigee_client_id}, callback=self.parse_jwk)
    ...
    def parse(self, response: Response) -> Iterable[Feature]:
        ...
        for location in response.json()["data"]["stores"]:
            ...
            if "urlRegion" in location:
                item["website"] = website_prefix + location["urlRegion"] + "/" + location["displayName"].lower().replace(" ", "-")
