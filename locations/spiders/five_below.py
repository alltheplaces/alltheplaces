...
class FiveBelowSpider(scrapy.Spider):
    ...
    def parse_hours(self, response):
        ...
        if store:
            ...
            all_hours = store.xpath(
                '//*/div[@class="Core-hours"]//*[@itemprop="openingHours"]'
            )
            regex = re.compile(
                r"(Su|Mo|Tu|We|Th|Fr|Sa)\s+(\d{2}:\d{2})\s*-(\d{2}:\d{2})"
            )
            for hours in all_hours:
                ...
                if match:
                    ...
                    opening_hours.add_range(
                        day=day_of_week, open_time=open_time, close_time=close_time
                    )
