            ...
            if custom_field["name"] in DAYS_FULL:
                ...
                if "," in custom_field["value"]:
                    for time_range in custom_field["value"].split(","):
                        item["opening_hours"].add_range(DAYS_EN[custom_field["name"]], *time_range.strip().split(" - ", 1))
                else:
                    item["opening_hours"].add_range(DAYS_EN[custom_field["name"]], *custom_field["value"].split(" - ", 1))
