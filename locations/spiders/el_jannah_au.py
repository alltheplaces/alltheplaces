...
class ElJannahSpider(Spider):
    ...
    def parse(self, response):
        ...
        locations_geodata_raw = re.findall(r"\[\s*(\d+)\s*\,\s*'\s*([^']+)\s*'\s*\,\s*'\s*(-?\d{2,3}\.\d+)\s*'\s*\,\s*'\s*(-?\d{2,3}\.\d+)\s*'\s*\]", js_blob)
        ...
        for location in response.xpath('//div[@class="location-item"]'):
            properties = {
                ...
                "addr_full": location.xpath('./div[1]/div[@class="location-item-row"][1]/div[@class="location-item-text"]/text()').get("").strip(),
                "opening_hours": OpeningHours(),
            }
            properties["lat"] = locations_geodata_dict[properties["ref"]][1]
            properties["lon"] = locations_geodata_dict[properties["ref"]][2]
            hours_string = " ".join(filter(None, map(str.strip, location.xpath('./div[1]/div[@class="location-item-row"][2]/div[@class="location-item-text"]//text()').getall())))
