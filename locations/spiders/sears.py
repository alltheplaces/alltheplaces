...
class SearsSpider(scrapy.Spider):
    ...
    def parse_sears_hts(self, response):
        response.selector.remove_namespaces()
        base_url = 'http://www.searshometownstores.com/hometown/services/Store.Service.ss?'
        for url in response.xpath('//loc/text()').extract():
            ...
            yield scrapy.http.Request(base_url + urlencode(params), callback=self.parse_sears_hts_store)
    ...
    def parse_sears_store(self, response):
        ...
        if response.request.meta.get('redirect_urls'):
            ...
            store_script_data  = response.xpath('//script[contains(text(),"var storeTitle")]/text()').extract_first()
