...
class PapaMurphysSpider(scrapy.Spider):
    ...
    def parse_stores(self, response):
        properties = {
            'addr_full': response.xpath('//div[@class="streetaddress"]//span[@class="street-address"]/text()').extract_first(),
            'phone': response.xpath('(//dl[@class="MenuInfo"]/dd/span[@class="tel"]/text())').extract_first().strip(),
            'city': response.xpath('//div[@class="streetaddress"]//span[@class="locality"]/text()').extract_first(),
            'state': response.xpath('//div[@class="streetaddress"]//span[@class="region"]/text()').extract_first(),
            'postcode': response.xpath('//div[@class="streetaddress"]//span[@class="postal-code"]/text()').extract_first(),
            'country': 'US',
    ...
    def parse_state_stores(self, response):
        stores = response.xpath('//ul[@id="ParticipatingRestaurants"]//li//a/@href').extract()
        for store in stores:
