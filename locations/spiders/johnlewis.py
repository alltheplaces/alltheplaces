...
class JohnLewisSpider(SitemapSpider):
    ...
    sitemap_rules = [(r"https:\/\/www\.johnlewis\.com\/our\-shops\/([-\w]+)", "parse_stores")]
    ...
    def parse_stores(self, response):
        ...
        location = json.loads(response.xpath('//script[@id="jsonPageData"]/text()').get())
        if location:
            item["lat"] = location["latitude"]
            item["lon"] = location["longitude"]

        addr_full = response.xpath('//p[@class="shop-details-address"]/text()').get().replace("  ", " ")
        postcode_re = re.match(r"([: \w]+) (\w{1,2}\d{1,2}\w? \d\w{2})", addr_full)
        if postcode_re:
            item["postcode"] = postcode_re.group(2)
            addr_full = ", ".join([postcode_re.group(1), postcode_re.group(2), "United Kingdom"])
        ...
        opening_rules = response.xpath('//dl[@class="opening-weeks-list"]/descendant::*/text()').getall()
