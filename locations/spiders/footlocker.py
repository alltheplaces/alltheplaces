...
class FootLockerSpider(scrapy.Spider):
    ...
    def parse_store(self, response):
        ...
        ref = response.xpath('//*[@itemtype="http://schema.org/ShoeStore"]/@itemid').extract_first().split('#')[1]

        # The name can be split across multiple child elements so we need to join them together here.
        name = ''.join(response.xpath('//*[@itemprop="name"][@id="location-name"]//text()').extract())
