...
class ARB4x4AccessoriesSpider(Spider):
    ...
    item_attributes = {"brand": "ARB 4×4 Accessories", "brand_wikidata": "Q126166453", "extras": Categories.SHOP_CAR_PARTS.value}
    ...
    def parse(self, response):
        for location in response.json():
            ...
            if item["website"] and not item["website"].startswith("www.") and not item["website"].startswith("https://") and not item["website"].startswith("http://"):
            ...
            hours_string = " ".join(filter(None, map(str.strip, Selector(text=location.get("opening_hours")).xpath('//text()').getall())))
