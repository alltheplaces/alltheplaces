...
class CostcoCAGBUSSpider(JSONBlobSpider, CamoufoxSpider):
    ...
    start_urls = ["https://ecom-api.costco.com/warehouseLocatorMobile/v1/warehouses.json?client_id=45823696-9189-482d-89c3-0c067e477ea1&latitude=0&longitude=0&limit=5000&distanceUnit=km"]
    ...
    def post_process_item(self, item: Feature, response: Response, feature: dict) -> Iterable[Feature]:
        ...
        if feature.get("subType") and feature["subType"].get("code") == "Business Center":
            ...
            item["website"] = "https://www.costcobusinessdelivery.com/warehouse-locations/{}-{}-{}-{}.html".format(slug_branch_name, slug_city_name, slug_state_code, item["ref"])
        else:
            # Costco allowing retail customers
            if item["country"] == "US":
                item["website"] = "https://www.costco.com/warehouse-locations/{}-{}-{}.html".format(slug_branch_name, slug_state_code, item["ref"])
            elif item["country"] == "CA":
                item["website"] = "https://www.costco.ca/warehouse-locations/{}-{}-{}.html".format(slug_branch_name, slug_state_code, item["ref"])
        ...
        for day_hours in feature.get("hours", []):
            ...
            for day_number in day_hours["weekDays"]:
                item["opening_hours"].add_range(DAYS_FROM_SUNDAY[day_number - 1], day_hours["open"], day_hours["close"], "%H:%M:%S")
