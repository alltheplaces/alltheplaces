...
class DicksSportingGoodsSpider(scrapy.Spider):
    ...
    item_attributes = {"brand": "Dick's Sporting Goods"}
    ...
    def parse_hours(self, response):
        ...
        start_times = response.xpath(
            '//meta[@property="business:hours:start"]/@content'
        ).extract()
        end_times = response.xpath(
        ...
        for day, open_time, close_time in zip(days, start_times, end_times):
            opening_hours.add_range(
                day=DAY_MAPPING[day], open_time=open_time, close_time=close_time
            )
    ...
    def parse_store(self, response):
        ...
        shopping_center = string.capwords(
            "".join(
                response.xpath(
                    '//div[contains(@class, "shopping_center")]/text()'
                ).extract()
            ).strip()
        )
        if shopping_center:
