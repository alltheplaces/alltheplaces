...
class TheCoffeeBeanAndTeaLeafSGSpider(AmastyStoreLocatorSpider):
    ...
    def parse_item(self, item, location, popup_html):
        ...
        item["addr_full"] = " ".join((" ".join(popup_html.xpath('//div[contains(@class, "amlocator-info-popup")]/div[contains(@class, "amlocator-image")]/following-sibling::text()').getall())).split())
        item["website"] = popup_html.xpath('//a[contains(@class, "amlocator-link")]/@href').get()

        oh = OpeningHours()
        hours_raw = " ".join((" ".join(popup_html.xpath('//div[contains(@class, "amlocator-description")]//text()').getall())).split()).replace("24 Hours", "12:00am to 11:59pm").replace("Operating Daily,", "Mon-Sun")
