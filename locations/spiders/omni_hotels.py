...
ca_states = ["Alberta", "British Columbia", "Manitoba", "New Brunswick", "Newfoundland and Labrador", "Nova Scotia",
             "Ontario", "Prince Edward Island", "Quebec", "Saskatchewan", "Northwest Territories", "Nunavut", "Yukon"]
mx_states = ["Aguascalientes", "Baja California Norte", "Baja California Sur", "Campeche", "Chiapas", "Mexico City",
...
class OmniHotels(scrapy.Spider):
    download_delay = 0.2
    name = "omni_hotels"
    item_attributes = { 'brand': "Omni Hotels" }
    ...
    def parse_location(self, response):
        try:
            ...
            properties = {
                'addr_full': response.xpath('//div[@class="plp-hotel-content-container"]/p/a/text()').extract_first().split('  ')[-1],
                'city': response.xpath('//div[@class="plp-hotel-content-container"]/p/a//text()[2]').extract_first(),
                'state': the_state,
                'postcode': response.xpath('//div[@class="plp-hotel-content-container"]/p/a//text()[4]').extract_first().split('  ')[-1],
                'ref': response.url.split('/')[-1],
                'website': response.url,
                'phone': response.xpath('//div[@class="plp-hotel-content-container"]/p[2]/a/@aria-label').extract()[0],
                'name': response.xpath('//div[@class="plp-resort-title-container"]/h1/@data-ol-has-click-handler').extract(),
                'country': the_country,
