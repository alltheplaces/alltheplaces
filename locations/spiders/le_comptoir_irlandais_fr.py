...
from locations.hours import DAYS_FR, OpeningHours, DELIMITERS_FR, CLOSED_FR
...
class LeComptoirIrlandaisFRSpider(Spider):
    ...
    def parse(self, response):
        ...
        for store in stores:
            ...
            item["branch"] = store.attrib.get("name").removeprefix("Le Comptoir Irlandais ").removeprefix("de ").removeprefix("du ").removeprefix("d'")
            ...
            for row in selector.xpath('//table//tbody/tr'):
                ...
                item["opening_hours"].add_ranges_from_string(day + " " + hours.replace("h30",":30").replace("h",":00"), DAYS_FR, delimiters=DELIMITERS_FR, closed=CLOSED_FR)
            yield item 
