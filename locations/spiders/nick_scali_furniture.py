...
class NickScaliFurnitureSpider(AmastyStoreLocatorSpider):
    ...
    def parse_item(self, item, location, popup_html):
        item["addr_full"] = " ".join((" ".join(popup_html.xpath('//div[contains(@class, "amlocator-info-popup")]/text()').getall())).split())
        yield Request(url=item["website"], meta={"item": item}, callback=self.add_hours)

    def add_hours(self, response):
        item = response.meta["item"]
        oh = OpeningHours()
        hours_raw = (" ".join(response.xpath('//div[contains(@class, "amlocator-schedule-table")]/div/span/text()').getall()).replace(" - ", " ").replace(" am", "AM").replace(" pm", "PM")).split()
