...
class DeloitteSpider(scrapy.Spider):
    ...
    def start_requests(self):
        start_urls = (
            ...
            ('https://www2.deloitte.com/us/en/footerlinks/global-office-directory.html', self.parse_global)
        )
    ...
    def parse_location(self, response):
        ...
        try:
            ...
            try:
                ...
                map_url = store_data.get("hasMap", '').replace(u'\u003d', '=')
                try:
                    lat, lon = re.search(r'll=([\d\.-]+),([\d\.\-]+)', map_url).groups()
                    lat, lon = float(lat), float(lon)
                except AttributeError:
                    lat, lon = None, None  # gmap url is a /place/<address> type url (no lat/lon available)
            ...
            properties.update({
                ...
                'phone': phone,
                'addr_full': address,
                'city': store_data["address"].get("addressLocality", "").strip().replace(u'\u200b', '') or None,
                'state': store_data["address"].get("addressRegion", "").strip().replace(u'\u200b', '') or None,
                'postcode': store_data["address"].get("postalCode", "").strip().replace(u'\u200b', '') or None,
