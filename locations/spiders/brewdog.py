...
class BrewdogSpider(scrapy.Spider):
    ...
    def parse(self, response):
        yield from response.follow_all(css="a.bar-listing__results__bar__buttons__view", callback=self.parse_bar)

    def parse_bar(self, response):
        name = response.xpath('//meta[@name="title"]/@content').extract_first()
        addr = next(s for s in response.xpath('//div[@id="contact"]//text()').extract() if s.strip())
        ...
        for row in response.xpath('//div[@id="opening-hours"]/p/text()').extract()[:7]:
            ...
            if groups is None:
                continue
            day = groups["day"][:2]
            opening_hours.add_range(day, groups["open_time"], groups["close_time"], "%I:%M%p")

        map_url = response.xpath('//iframe[@class="bar-detail__map"]/@src').extract_first()
        if map_url:
