...
class MarcusTheatresSpider(scrapy.Spider):
    ...
    start_urls = (
        'http://www.marcustheatres.com/theatre-locations/',
    )
    ...
    def parse_store(self, response):
        properties = {
            ...
            'lat': float(response.xpath('//div[@class="map-link"]/a/@href').extract_first().split('loc:')[1].split('+')[0]),
            'lon': float(response.xpath('//div[@class="map-link"]/a/@href').extract_first().split('loc:')[1].split('+')[1]),
        }
