...
class MarcusTheatresSpider(scrapy.Spider):
    name = "marcustheatres"
    item_attributes = {"brand": "Marcus Theatres"}
    ...
    def parse_store(self, response):
        properties = {
            ...
            "addr_full": response.xpath(
                '//div[@class="theatre-map__street-address"]/text()'
            ).extract_first(),
            "city": response.xpath(
                '//div[@class="theatre-map__locality"]/text()'
            ).extract_first(),
            "state": response.xpath(
                '//div[@class="theatre-map__region"]/text()'
            ).extract_first(),
            "postcode": response.xpath(
                '//div[@class="theatre-map__postal-code"]/text()'
            ).extract_first(),
            "phone": response.xpath(
                '//div[@class="theatre-map__theatre-phone"]/text()'
            ).extract_first(),
            "website": response.url,
            "lat": float(
                response.xpath('//div[@class="map-link"]/a/@href')
                .extract_first()
                .split("loc:")[1]
                .split("+")[0]
            ),
