        ...
        for location in response.json()["response"]["entities"]:
            ...
            for day_hours in location["profile"]["hours"]["normalHours"]:
                ...
                for interval in day_hours["intervals"]:
                    item["opening_hours"].add_range(day_hours["day"].title(), str(interval["start"]).zfill(4), str(interval["end"]).zfill(4), "%H%M")
            
            apply_yes_no(PaymentMethods.AMERICAN_EXPRESS, item, "American Express" in location["profile"]["paymentOptions"], False)
            ...
            apply_yes_no(PaymentMethods.DINERS_CLUB, item, "Diners Club" in location["profile"]["paymentOptions"], False)
            ...
            apply_yes_no(PaymentMethods.SAMSUNG_PAY, item, "Samsung Pay" in location["profile"]["paymentOptions"], False)
