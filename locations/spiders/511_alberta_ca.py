...
class FivehundredelevenAlbertaCASpider(JSONBlobSpider):
    name = "511_alberta_ca"
    item_attributes = {"operator": "Alberta Ministry of Transportation", "operator_wikidata": "Q15058324", "extras": Categories.SURVEILLANCE_CAMERA.value}
    ...
    def post_process_item(self, item: Feature, response: Response, feature: dict) -> Iterable[Request]:
        ...
        yield Request(url="https://511.alberta.ca/tooltip/Cameras/{}?lang=en-US".format(item["ref"]), callback=self.parse_cameras, meta={"item": item})
