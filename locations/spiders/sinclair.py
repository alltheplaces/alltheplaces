...
class SinclairSpider(scrapy.Spider):
    ...
    def start_requests(self):
        yield scrapy.Request('https://www.sinclairoil.com/location-feed',
                             callback=self.add_locations)

    def add_locations(self, response):
        yield scrapy.Request('https://www.sinclairoil.com/truck-stop-feed',
                             callback=self.add_truck_stops,
                             meta={
                                 'locations': self.parse_location_feed(response, 'general')
                             })

    def add_truck_stops(self, response):
        yield scrapy.Request('https://www.sinclairoil.com/gold-truck-stop-feed',
                             callback=self.add_gold_truck_stops,
                             meta={
                                 'locations': response.meta['locations'] + self.parse_location_feed(response, 'truck_stop')
                             })

    def add_gold_truck_stops(self, response):
        yield scrapy.Request('https://www.sinclairoil.com/customers/location-list?page=0',
                             meta={
                                 'locations': response.meta['locations'] + self.parse_location_feed(response, 'truck_stop')
                             })
    ...
    def parse(self, response):
        ...
        for row in response.css('.views-table tbody tr'):
            ...
            address = row.css(
                '.views-field-field-physical-address::text').get().strip()
            city = row.css('.views-field-field-city::text').get().strip()
            state = row.css('.views-field-field-state::text').get().strip()
            postcode = row.css(
                '.views-field-field-zip-code::text').get().strip()
            phone = row.css(
                '.views-field-field-phone-number::text').get().strip()
            ...
            try:
                location = next(l for l in locations if (
                    address in l['description'] and postcode in l['description']))
        ...
        if last_page_url:
            ...
            yield scrapy.Request(f"https://www.sinclairoil.com/customers/location-list?page={current_page + 1}",
                                 meta=response.meta)
