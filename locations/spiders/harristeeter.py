...
class HarristeeterSpider(scrapy.Spider):
    name = "harristeeter"
    allowed_domains = ["harristeeter.com","webhook.site"]
    start_urls = (
    ...
    def parse(self, response):
        ...
        yield scrapy.Request('https://www.harristeeter.com/api/v1/stores/search?Address=98011&Radius=20000&AllStores=true', headers=headers,cookies=cookies, callback=self.parse_shop)


    def parse_shop(self, response):
        answers=json.loads(response.text)['Data']

        for answ in answers:
            props = {
                'opening_hours' : answ['StoreHours'].replace('Open 24 hrs','Mo-Su 0:00-24:00'),
                'phone' : answ['Telephone'],
                'country' : answ['Country'],
                'ref' : answ['Title'],
                'addr_full' : answ['Street'],
                'lat' : float(answ['Latitude']),
                'lon' : float(answ['Longitude']),
                }

            try:
                props['postcode'] = answ['ZipCode']
            except Exception as e:
                postcode=''

            try:
                props['city'] = answ['City']
            except Exception as e:
                city=''

            try:
                props['state'] = answ['State']
            except Exception as e:
                state=''
