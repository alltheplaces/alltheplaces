...
class IkeaUKSpider(scrapy.Spider):
    ...
    start_urls = ("http://www.ikea.com/gb/en/store/",)

    def store_hours(self, store_hours):
        opening_hours = OpeningHours()
        store_hours = store_hours.xpath(
            './/h3[contains(strong, "Store")]/following-sibling::p[1]'
        )
        weekdays = store_hours.xpath(".//strong/text()").extract()
        weekday_hours = store_hours.xpath(
        ...
        for weekday in weekday_data:
            ...
            day_range = (
                day_range.strip().strip("\u200b").strip(":").strip(" -").split(" - ")
            )
    ...
    def parse(self, response):
        shops_urls = response.xpath(
            '//article//a[contains(@href, "stores")]/@href'
        ).extract()
    ...
    def parse_shops(self, response):
        ...
        address_full = response.xpath(
            '//h3[contains(strong, "Address")]/following-sibling::p/text()'
        ).extract_first()
        name = response.xpath(
            '//h1[@class="c1m1sl8e pub__h1 s1gshh7t"]/text()'
        ).extract_first()
        ...
        hours = response.xpath(
            './/h2[contains(strong, "Opening hours")]/following::div[1]'
        )
