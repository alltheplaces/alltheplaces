...
class CoventryBuildingSocietyGBSpider(Spider):
    ...
    def parse(self, response, **kwargs):
        for location in response.json()["branches"]:
            ...
            item["street_address"] = merge_address_lines([location["address"].get("address1"), location["address"].get("address2")])
            if "html" in location["branchdetails"]["branchnumber"]:
                item["branch"] = location["branchdetails"]["branchnumber"].replace("/content/cbs/member/en/our-branches/","").replace(".html","")
                item["website"] = "https://www.coventrybuildingsociety.co.uk/member/our-branches/"+item["branch"]+".html"
