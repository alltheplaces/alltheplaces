...
class RASushiUSSpider(SitemapSpider):
    ...
    def parse_store(self, response):
        properties = {
            ...
            "addr_full": " ".join(filter(None, response.xpath("//div[contains(@class, 'store_locator_single_address') or contains(@class, 'address')]/text()").getall())).strip(),
        ...
        if restaurant:
            hours_string = " ".join(filter(None, response.xpath('//section[contains(@class, "hours_wrapper")]//ul/li[1]//p/text()').getall())).strip().upper().replace("MIDNIGHT", "12:00AM")
