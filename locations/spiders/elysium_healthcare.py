...
class ElysiumHealthcareSpider(scrapy.Spider):
    ...
    def parse_location(self, response):
        coming_soon = response.xpath(
            '//h1[@class="elementor-heading-title elementor-size-default"]/span/text()').extract_first()
        clutter = ['or', 'or ', 'Email us', 'Email us ', '\xa0', 'or\xa0']

        if not coming_soon:  # Skip empty pages
            ref = re.search(r'.+/(.+?)/?(?:\.html|$)', response.url).group(1)
            name = response.xpath('(//h1[@class="elementor-heading-title elementor-size-default"]/text())[2]').extract_first()
            unit_addr = response.xpath('//div[contains(@class, "contact-link-small")]/div/p[not(a) and not(strong) and not(contains(text(), "Tel:"))]//text()').extract()
            if unit_addr:  # Incorporate unit/building identifier
                addr_first_line = response.xpath(
                    '//div[contains(@class, "contact-link-small")]/div/div/p/text()').extract_first()
                ...
                addr_first_line = response.xpath('//div[contains(@class, "contact-link-small")]/div/div/p/text()').extract_first()
            addr_last_line = response.xpath('(//div[contains(@class, "contact-link-small")]//p[not(a) and not(strong)]/text())[2]').extract_first()
            if addr_first_line and addr_last_line:  # Address in two parts
                # Check for formatting common on Welsh pages
                welsh_ll_addr = response.xpath('(//div[contains(@class, "contact-link-small")]//p[not(a) and not(strong) and not(contains(text(), "Tel:"))]/text())[3]').extract_first()
