...
class RunzaUSSpider(JSONBlobSpider):
    name = "runza_us"
    start_urls = ["https://drupal.runza.com/api/locations?proximity[value]=15&proximity[source_configuration][origin][lat]=&proximity[source_configuration][origin][lon]="]

    def post_process_item(self, item, response, location):
        item["lat"] = location["field_lat_lon"][0]["lat"]
        item["lon"] = location["field_lat_lon"][0]["lon"]
        item["street_address"] = merge_address_lines([location["field_address"][0]["address_line1"], location["field_address"][0]["address_line2"]])
        ...
        if location["field_hide_dining_room_hours"][0]["value"] is False:
            ...
            for day in range(0, 7):
                ...
                if day_hours:
                    oh.add_range(day=DAYS[day], open_time=f"{day_hours['starthours']}", close_time=f"{day_hours['endhours']}", time_format="%H%M")
