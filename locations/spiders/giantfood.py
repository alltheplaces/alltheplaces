...
class GiantFoodSpider(scrapy.Spider):
    ...
    def parse_store(self, response):
        store_number = response.xpath('//div[contains(@class, "StoreDetails-storeNum")]/text()').extract()[-1].strip('#')
        properties = {
            'ref': store_number,
        ...
        hours = self.parse_hours(response.xpath('//div[contains(@class, "StoreDetails")]//tr[@itemprop="openingHours"]/@content').extract())
        if hours:
        ...
        for hour in hours:
            if "All Day" in hour:
                ...
                day, open_time, close_time = re.search(
                    r'([a-z]{2})\s([0-9:]+)-([0-9:]+)', hour, flags=re.IGNORECASE).groups()
