...
class BedBathNTableSpider(AmastyStoreLocatorSpider):
    ...
    def start_requests(self):
        for allowed_domain in self.allowed_domains:
            ...
            yield Request(url=f"https://{allowed_domain}/locator", meta={"allowed_domain": allowed_domain}, callback=self.scrape_data_from_locator_page)
    ...
    def scrape_data_from_locator_page(self, response):
        ...
        for location in locations_html:
            ...
            item["street_address"] = ", ".join(filter(None, location.xpath('.//div[@class="store-address"]//text()').getall())).strip()
            item["addr_full"] = unquote_plus(location.xpath('.//a[@class="get-direction"]/@href').get().split('//', 2)[2])
            item["phone"] = location.xpath('.//div[@class="phone"]/text()').get().strip()
            hours_string = " ".join(location.xpath('.//div[@class="amlocator-schedule-table"]/div/span/text()').getall())
