...
class CycleSpotJPSpider(CrawlSpider):
    ...
    rules = [
        Rule(LinkExtractor(allow=r"^https:\/\/www\.cyclespot\.net\/[^/]+\/?$", restrict_xpaths='//div[@id="prefecture"]//a'), callback=None, follow=True),
    ...
    def parse(self, response):
        ...
        properties = {
            ...
            "addr_full": re.sub(r"\s+", " ", response.xpath('//div[@class="cs_wpsl_shopinfomation"]/p[2]/text()').get().strip()),
            "phone": re.search(r"([\d\-.]{8,})", " ".join(response.xpath('//div[@class="cs_wpsl_shopinfomation"]/p/i[contains(@class, "fa-phone")]/parent::p//text()').getall())).group(1),
