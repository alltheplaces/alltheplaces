...
class AldiLUSpider(scrapy.Spider):
    ...
    start_urls = (
        'https://de.aldi.lu/filialen/index/page-1',
    )
    ...
    def parse(self, response):
        ...
        for store in stores:
            ...
            website = 'https://de.aldi.lu' + data.css('a::attr(href)').extract_first()[1:]
            ref = website.split('/')[-2]
        ...
        if next:
            yield scrapy.Request(
                'https://de.aldi.lu' + next.extract_first()[1:],
                callback = self.parse
            )
