...
class HalfPriceBooksSpider(scrapy.Spider):
    ...
    item_attributes = { 'brand': "Half Price Books" }
    ...
    def parse_store(self, response):
        ...
        address = response.xpath('//div[@class="font-large font-pn-reg"]/div[1]/text()').extract_first().strip()
    ...
    def parse_hours(self, elements):
        ...
        for days in elements:
            if days.split()[0] not in ['Mon','Tue','Wed','Thu','Fri','Sat','Sun','Every']:
                continue
            if len(re.findall(r'(-)',days)) > 1:
                day_start, day_end = [day_mapping[days.split()[0]],day_mapping[days.split()[2]]]
