...
class ChrisWillemseCyclesZASpider(Spider):
    ...
    def parse(self, response: Response) -> Iterable[Feature]:
        locations = parse_js_object(response.xpath('//script[contains(text(), "var markers = ")]/text()').get().split("var markers =")[1])
        ...
        if len(locations) != 1:
            raise RuntimeError("This chain used to have more than one location and shared the same spider as parent brand with spider cycle_lab_za. Then it reduced to one location and the store locator page is no longer the same as cycle_lab_za. This new store locator page and this spider only supports a single location being returned. It appears though that this chain has expanded again to have more than one store. This spider thus needs to be rewritten.")
        properties = {
            "branch": locations[0][0],
            "addr_full": merge_address_lines(response.xpath('//div[@class="Store-Location-Address"]/div[1]/div[1]/p//text()').getall()),
            "phone": response.xpath('//div[@class="Store-Location-Address"]/div[1]/div[1]/div[@class="number"]/h3/text()').get(),
            "opening_hours": OpeningHours(),
        }
        hours_text = " ".join(response.xpath('//div[@class="hours-location"]//text()').getall()).replace("Saturdays", "Saturday")
