...
class EasyBathroomsGBSpider(CrawlSpider):
    ...
    item_attributes = {"brand": "Easy Bathrooms", "brand_wikidata": "Q114348566", "extras": Categories.SHOP_BATHROOM_FURNISHING.value}
    ...
    def parse(self, response):
        ...
        properties = {
            ...
            "lat": lat,
            "lon": lon,
            "addr_full": ", ".join(filter(None, map(str.strip, address_details.xpath('./div[2]/ul/li/text()').getall()))),
        ...
        hours_string = " ".join(filter(None, map(str.strip, response.xpath('//div[@id="opening-times"]/ul[1]//text()').getall())))
