...
class UniversityMarylandMedicalSystemSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        paths = response.xpath(
            '//select[@class="select-search__select"]//option/@value'
        ).extract()

        for path in paths:
            yield scrapy.Request(
                url=template.format(path=path), callback=self.parse_list
            )

    def parse_list(self, response):
        list_urls = response.xpath(
            '//li[@class="search-results__item u-cf"]/div[2]/div/a[1]/@href'
        ).extract()
    ...
    def parse_location(self, response):
        data = json.loads(
            response.xpath(
                '//div[contains(@class, "locations")]/@data-map-json'
            ).extract_first()
        )
