...
class HertzSpider(Spider):
    ...
    def parse(self, response: Response, **kwargs: Any) -> Any:
        for country_data in response.json()["data"]["model"]:
            ...
            yield scrapy.Request(url= "https://api.hertz.com/rest/geography/city/country/{}".format(country_code),callback=self.parse_city,cb_kwargs={"country_name":country_name,"country_code":country_code})

    def parse_city(self,response,**kwargs):
        for city_info in response.json()["data"]["model"]:
            city_name = city_info["name"]
            yield scrapy.Request(url= "https://api.hertz.com/rest/location/country/{}/city/{}".format(kwargs["country_code"],city_name),callback=self.parse_details)
    ...
    def parse_details(self,response,**kwargs):
        for shop in response.json()["data"]["locations"]:
            ...
            item["website"] = "/".join(["https://www.hertz.com/us/en/location",shop["country_name"],shop["city"],shop["extendedOAGCode"]])
            item["street_address"] = shop.get("streetAddressLine1")
            if shop.get("streetAddressLine2",""):
                item["addr_full"] = "".join([shop.get("streetAddressLine1",""),shop.get("streetAddressLine2",""),shop.get("streetAddressLine3","")])
