...
from locations.hours import OpeningHours, CLOSED_DE, DAYS_DE
...
class VolgCHSpider(Spider):
    ...
    start_urls = ["https://www.volg.ch/standorte-oeffnungszeiten/?tx_kochvolgstores_storelist[action]=search&tx_kochvolgstores_storelist[controller]=Stores"]
    ...
    def parse_store_list(self, response: Response) -> Iterable[Request]:
        for store_slug in list(map(lambda x: x.split("?", 1)[0], response.xpath('//div[contains(@class, "c-location-list__link")]/a/@href').getall())):
            yield Request(url=urljoin("https://{}".format(self.allowed_domains[0]), store_slug), callback=self.parse_store)
    ...
    def parse_store(self, response: Response) -> Iterable[Feature]:
        properties = {
            ...
            "branch": response.xpath('//div[contains(@class, "c-content__header")]//h1/text()').get().removeprefix("Volg "),
            "lat": response.xpath('//@data-lat').get(),
            "lon": response.xpath('//@data-lng').get(),
            "addr_full": merge_address_lines(response.xpath('//div[contains(@class, "c-content__main")]/section[2]/div[contains(@class, "container")]/div[1]/div[2]/div[1]/div[1]/p[1]/text()').getall()),
            "phone": response.xpath('//div[contains(@class, "c-content__main")]/section[2]/div[contains(@class, "container")]/div[1]/div[2]/div[1]/div[1]/p[2]/a/@href').get().removeprefix("tel:"),
            "website": response.url,
            "opening_hours": OpeningHours(),
        }
        hours_string = re.sub(r"\s+", " ", " ".join(response.xpath('//table[contains(@class, "c-location-hours")]//td[position() = 1 or position() = 3]/text()').getall()))
