...
class SmashburgerSpider(CrawlSpider, StructuredDataSpider):
    ...
    rules = [
        Rule(LinkExtractor(allow=r"^https:\/\/smashburger\.com\/locations\/[a-z]{2}\/[a-z]{2}(?:\/(?:[^/]+\/?))?$"), follow=True),
        Rule(LinkExtractor(allow=r"^https:\/\/smashburger\.com\/locations\/[a-z]{2}\/[a-z]{2}\/[^/]+\/.+"), callback="parse_sd", follow=False),
    ]
