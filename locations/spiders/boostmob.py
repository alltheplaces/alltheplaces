...
class BoostMobileSpider(scrapy.Spider):
    ...
    start_urls = (
        'https://www.boostmobile.com/locations.html',
    )
    ...
    def parse(self, response):
        with open('./locations/searchable_points/us_zcta.csv') as zips:
            ...
            for zip in zips:
                row = zip.replace('\n', '').replace('"', '')
                searchurl = 'https://boostmobile.nearestoutlet.com/cgi-bin/jsonsearch-cs.pl?showCaseInd=false&brandId=bst&results=50&zipcode={pc}'.format(pc=row)
    ...
    def parse_search(self, response):
        ...
        for i in data['nearestOutletResponse']['nearestlocationinfolist']['nearestLocationInfo']:
