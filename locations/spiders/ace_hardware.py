...
class AceHardwareSpider(scrapy.Spider):
    ...
    def parse_hours(self, lis):
        ...
        for day in DAY_MAPPING:
            if 'openingTime'+day in lis and 'closingTime'+day in lis:
                ...
                clean_time = clean_time + day[:2]+' '+open_time[0:2]+':'+open_time[2:]+"-"+close_time[0:2]+':'+close_time[2:]+' ;'
        return clean_time

    def parse_stores(self, response):
        id=response.meta['id']
        json_data = json.loads(response.body_as_unicode())
        if not 'address1' in json_data:
            return
        properties = {
            ...
            'ref': id,
            'website': "http://www.acehardware.com/mystore/index.jsp?store="+id,
            'lat': float(json_data['postalCode']),
    ...
    def parse(self, response):
        ...
        for store_id in store_ids:
            ...
            yield scrapy.Request(url, callback=self.parse_stores,meta={'id':store_id})
