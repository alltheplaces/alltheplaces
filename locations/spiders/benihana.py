...
class BenihanaSpider(scrapy.Spider):
    ...
    def start_requests(self):
        yield scrapy.Request('https://www.benihana.com/wp-content/themes/grandrestaurant-child/js/locations.json',
    ...
    def parse(self, response):
        ...
        for url in urls:
            if url != 'http://www.dignityhealthsportspark.com/' or url != 'https://www.sprintcenter.com/':
                yield scrapy.Request(response.urljoin(url), callback=self.parse_stores)


    def parse_stores(self, response):
        delivery_only = response.xpath('//div[@class="hours-wrapper"]/p/strong/text()').extract_first()
        if delivery_only != 'DELIVERY HOURS':
            locationid = re.search(r'.*/locations/(.*)/$', response.url).groups()[0]
            location = [loc for loc in self.locations if loc["LocationId"] == locationid][0]
            properties = {
                ...
                'name': response.xpath('//h2[@class="ppb_title"]/text()').extract_first(),
