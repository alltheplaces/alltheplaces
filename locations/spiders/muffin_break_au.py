...
class MuffinBreakAUSpider(scrapy.Spider):
    ...
    def parse_hours(self, response):
        ...
        oh = OpeningHours()
        hours_raw = (" ".join(response.xpath('//table[contains(@class, "c-table")]/tr/td/text()').getall())).replace("Open 7 Days", "Mo-Su").replace("Thuesday", "Thursday").replace("8:300pm", "8:30pm")
