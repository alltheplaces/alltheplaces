...
class SpecsaversSpider(scrapy.Spider):
    ...
    def parse_hours(self, elements):
        ...
        for elem in elements:
            ...
            if hours[0] == "Closed":
                ...
                end_time = hours[2]
                opening_hours.add_range(
                    day=DAY_MAPPING[day], open_time=start_time, close_time=end_time
                )
    ...
    def parse_store(self, response):
        ...
        try:
            lat = float(
                response.xpath(
                    '//div[contains(@class,"aui-store-page")]/@data-store-geo-loc-lat'
                ).extract_first()
            )
            lon = float(
                response.xpath(
                    '//div[contains(@class,"aui-store-page")]/@data-store-geo-loc-lng'
                ).extract_first()
            )
        except:
            pass
        ...
        properties = {
            ...
            "phone": response.xpath(
                '//span[@class="contact--store-telephone--text"]/text()'
            ).extract_first(),
            "city": response.xpath(
            ...
            "lon": lon,
            "name": response.xpath(
                '//h1[@class="store-header--title"]/text()'
            ).extract_first(),
        }
