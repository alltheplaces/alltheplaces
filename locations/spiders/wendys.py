...
class WendysSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for url in response.xpath('//a[@class="Teaser-titleLink Link--big"]/@href').extract():
            yield scrapy.Request(response.urljoin(url))
        ...
        if response.xpath('//span/meta[@itemprop="longitude"]/@content'):
            properties = {
                ...
                'lon': response.xpath('//span/meta[@itemprop="longitude"]/@content').extract_first(),
                'lat': response.xpath('//span/meta[@itemprop="latitude"]/@content').extract_first(),
