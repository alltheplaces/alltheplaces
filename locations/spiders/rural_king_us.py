...
class RuralKingServiceCollectorSpider(scrapy.Spider):
    ...
    def parse_store_page(self, response):
        ...
        self.logger.info(f"Store #{store.get('storeName')} in {store.get('city')}, {store.get('stateOrProvinceName')} services: {services}")
...
class RuralKingUSSpider(scrapy.Spider):
    ...
    def errback_store_page(self, failure):
        ...
        if store:
            ...
            self.logger.warning(f"Failed to fetch store page for {store.get('city')}, {store.get('stateOrProvinceName')}. Using basic data.")
    ...
    def create_store_item(self, store, services, website_url=None):
        ...
        try:
            ...
            website = website_url if website_url else f"https://www.ruralking.com/storelocator/store/{store.get('uniqueID')}"
