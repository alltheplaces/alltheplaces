...
class TheGrooveTrainAUSpider(SitemapSpider):
    ...
    def parse(self, response):
        ...
        properties = {
            ...
            "phone": location_box.xpath('//div[contains(@class, "button-bar")]/div[contains(@class, "row1")]//p//text()').get().strip(),
            "website": response.url,
        }
        extract_google_position(properties, response)
        hours_string = " ".join(filter(None, location_box.xpath('//div[contains(@class, "trading-hours")]/div[contains(@class, "text")]/text()').getall()))
