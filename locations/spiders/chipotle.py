...
def build_links(tld, region_code, states):
    ...
    for state in states:
        ...
        start_url += "address=%s&distanceMode=1&region=%s&pageIndex=1&countPerPage=10&radius=500" % (state.strip(), region_code)
...
class ChipotleSpider(scrapy.Spider):
    ...
    def parse(self, response):
        """
            ...
            This throws a JSONDecodeError, at this point we stop parsing and nothing to yield nothing
        :param response:
        :return:
        """
        try:
        ...
        if json_data['restaurants']:
            for address in json_data['restaurants']:
                ...
                if link_id not in self.crawled_sites:
                    ...
                    logme(str(GeojsonPointItem(properties=properties, lon_lat=lon_lat,))+"\n\n", "crawl.txt")
    ...
    def process_hours(self, hours):
        ...
        for hr in hours:
            ...
            match_times = re.search(r"(\d{1,2}):(\d{1,2})\s(AM|PM)\s-\s(\d{1,2}):(\d{1,2})\s(AM|PM)$",
                                    open_times)
            result = []
            if match_times:
                m = match_times.groups()
                for day_range in day_ranges:
                    # let take care of the varying day of week
                    # they come in these variants Mon-Sat | Mon
                    day = list(map(lambda x: x.strip(), day_range.split("-")))
                    new_day_range = day[0][0:2] if len(day) == 1 else day[0][0:2] + "-" + day[1][0:2]
                    day_result = new_day_range + " " + str(int(m[0]) + self.am_pm(m[0], m[2])) + ":" + m[1] + "-" + \
                                 str(int(m[3]) + self.am_pm(m[3], m[5])) + ":" + m[4]
