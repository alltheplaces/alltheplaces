...
class LouisNielsenDKSpider(CrawlSpider):
    ...
    rules = [
        Rule(
            LinkExtractor(allow=r"^https:\/\/www\.louisnielsen\.dk\/find-din-butik\/(?!komplet-butiksoversigt)((?!<\/).+)$"),
    ...
    def parse_store(self, response):
        ...
        hours_raw = (" ".join(response.xpath('//tr[@itemprop="openingHours"]/@content').getall())).replace("Lukket", "0:00 - 0:00").replace(" - ", " ").split()
