...
class PearleVisionSpider(scrapy.Spider):
    ...
    def parse(self, response):
        coords = response.xpath(
            '//script[@type="text/javascript"]').extract()
        ...
        properties = {
            ...
            'city': re.search(r'^(.*),', response.xpath(
                'normalize-space(//*[@class="storeInfo"]/p/text()[2])').extract_first()).groups()[0],
            'state': re.search(r',\s([A-Z]{2})\s', response.xpath(
                'normalize-space(//*[@class="storeInfo"]/p/text()[2])').extract_first()).groups()[0],
            'postcode': re.search(r'[A-Z]{2}\s(.*)$', response.xpath(
                'normalize-space(//*[@class="storeInfo"]/p/text()[2])').extract_first()).groups()[0],
            'country': 'US',
            'lat': lat,
            'lon': lng,
