...
class MerriBekCityCouncilParksAUSpider(XMLFeedSpider):
    ...
    def parse_node(self, response: Response, selector: Selector) -> Iterable[Feature]:
        ...
        source_projection = int(selector.xpath("./Moreland:OpenSpace/Moreland:the_geom/gml:Point/@srsName").get().split("#", 1)[1])
        lon_source, lat_source = selector.xpath("./Moreland:OpenSpace/Moreland:the_geom/gml:Point/gml:coordinates/text()").get().split(",", 1)
