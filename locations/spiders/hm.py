    ...
    async def start(self) -> AsyncIterator[Request]:
        if self.use_hardcoded_countries:
            for country in GeonamesCache().get_countries():
                yield Request(self.country_url(country.lower()), callback=self.parse_country)
        else:
            yield Request("http://www.hm.com/entrance.ahtml", callback=self.parse)

    def parse(self, response):
        for country_code in response.xpath("//@data-location").getall():
            yield Request(self.country_url(country_code), callback=self.parse_country)
