from typing import Iterable

from scrapy.http import Response

from locations.categories import Categories, apply_category
from locations.items import Feature
from locations.storefinders.arcgis_feature_server import ArcGISFeatureServerSpider


class LauncestonCityCouncilStormwaterAssetsAUSpider(ArcGISFeatureServerSpider):
    name = "launceston_city_council_stormwater_assets_au"
    item_attributes = {"operator": "Launceston City Council", "operator_wikidata": "Q132860984", "state": "TAS"}
    host = "services.arcgis.com"
    context_path = "yeXpdyjk3azbqItW/ArcGIS"
    service_id = "Utilities"
    layer_id = "6"

    def post_process_item(self, item: Feature, response: Response, feature: dict) -> Iterable[Feature]:
        if status := feature.get("Status"):
            if status != "Active":
                return

        if gis_asset_id := feature.get("Gisassetid"):
            item["ref"] = str(gis_asset_id)
        else:
            item["ref"] = feature.get("Globalid")

        item["name"] = feature["Searchdescription"]

        if asset_type := feature.get("Class"):
            if asset_type.startswith("West Tamar "):
                item["operator"] = "West Tamar Council"
                item["operator_wikidata"] = "Q1665325"
                asset_type = asset_type.removeprefix("West Tamar ")
            match asset_type:
                case "Grated Pit" | "Gully Pit" | "Inlet Grate":
                    apply_category(Categories.KERB_GRATE, item)
                    item["extras"]["utility"] = "stormwater"
                    item["extras"]["substance"] = "rainwater"
                case "Manhole" | "Inspection Opening":
                    apply_category(Categories.MANHOLE, item)
                    item["extras"]["manhole"] = "drain"
                    item["extras"]["utility"] = "stormwater"
                    item["extras"]["substance"] = "rainwater"
                case "Pump Station":
                    apply_category(Categories.PUMPING_STATION_STORMWATER, item)
                    item["extras"]["utility"] = "stormwater"
                    item["extras"]["substance"] = "rainwater"
                case (
                    "Bend"
                    | "Detention Basin"
                    | "End of Line"
                    | "Gross Pit"
                    | "Headwall"
                    | "Junction"
                    | "Penstock Chamber"
                    | "Penstock Gate"
                    | "Sonde Point"
                    | "Tide Flap"
                    | "Valve"
                ):
                    return
                case _:
                    self.logger.warning("Unknown stormwater feature type: {}".format(feature["Class"]))
                    return

            yield item
