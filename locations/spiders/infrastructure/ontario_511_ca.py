...
class Ontario511CASpider(JSONBlobSpider):
    name = "ontario_511_ca"
    item_attributes = {
        "operator": "Ministry of Transportation of Ontario",
        "operator_wikidata": "Q3315416",
    ...
    def post_process_item(self, item: Feature, response: Response, feature: dict) -> Iterable[Request]:
        ...
        yield Request(
            url="https://511on.ca/tooltip/Cameras/{}?lang=en-US".format(item["ref"]),
    ...
    def parse_cameras(self, response: Response) -> Iterable[Feature]:
        ...
        for camera_link in response.xpath('//img[contains(@class, "carouselCctvImage")]'):
            ...
            camera["name"] = " ".join(
                filter(None, [camera_link.xpath("./@alt").get(), camera_link.xpath("./@title").get()])
            )
