...
class DynamicParcelDistributionSpider(scrapy.Spider):
    ...
    def shopsResults(self, response):
        ...
        for nr in range(lengthOfShops):
            ...
            formdata = {
                "ctl00$ctl16": "ctl00$ContentPlaceHolder1$modShopFinder$ctl01|ctl00$ContentPlaceHolder1$modShopFinder$repShopList$ctl0" + str(nr) + "$btnSelectShop",
    ...
    def openingHoursParse(self, response):
        ...
        if(len(shops) > 0):
            for nr in range(7):
                day = shops.css('span#ContentPlaceHolder1_modShopFinder_repBusinessHours_labBusinessDay_' + str(
                    nr) + '::text').extract()[0]
                openningHours = shops.css(
                    'span#ContentPlaceHolder1_modShopFinder_repBusinessHours_labBusinessHour_' + str(
                        nr) + '::text').extract()[0].split(' ')
