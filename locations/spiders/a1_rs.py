...
class A1RSSpider(Spider):
    ...
    def parse(self, response):
        cities_js = response.xpath('//script[contains(text(), "var cities_json = ")]/text()').get().split("var cities_json = ", 1)[1].split("}];", 1)[0] + "}]"
        cities_dict = parse_js_object(cities_js)
        for city in cities_dict:
            yield JsonRequest(url=f'https://a1.rs/sr_latin/api/pos/get_cities?city_id={city["id"]}', callback=self.parse_city_list)
