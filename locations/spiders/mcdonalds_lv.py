...
class McDonaldsLVSpider(scrapy.Spider):

    name = "mcdonalds_lv"
    brand = "McDonald's"
    allowed_domains = ["www.mcdonalds.lv"]
    start_urls = (
        'http://www.mcdonalds.lv/lv/restorani',
    )
    ...
    def parse(self, response):
        ...
        for store in stores:
            ...
            name = store.xpath('.//div[@class="title"]/h4/text()').extract_first().strip()
            lat, lon = self.parse_latlon(store.xpath('.//div[@class="col-info-lft"]/div/a/@href').extract_first().strip())
