...
class FreshChoiceNZSpider(CrawlSpider):
    ...
    def parse(self, response):
        properties = {
            ...
            "addr_full": re.sub(r"\s+", " ", " ".join(response.xpath('//div[@class="Footer__Column"]//a[contains(@href, "maps.google.com")]/text()').getall())).strip(),
            "phone": response.xpath('//div[@class="Footer__Column"]//a[contains(@href, "tel:")]/@href').get("").replace("tel:", "").strip(),
            "email": response.xpath('//div[@class="Footer__Column"]//a[contains(@href, "mailto:")]/@href').get("").replace("mailto:", "").strip(),
            "website": response.url,
        }
        hours_string = " ".join(response.xpath('//div[@class="Footer__Column"]//dl[@class="TradingHours__Days"]//text()').getall()).replace("day ", "day: ")
