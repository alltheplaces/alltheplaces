...
class LesliesPoolmartSpider(scrapy.Spider):
    ...
    item_attributes = {'brand': "Leslie's Poolmart"}
    ...
    def parse_store(self, response):
        ...
        if store_title:
            ...
            if city_state_postal:  # Get store details, otherwise pass if empty page
                city, state, zipcode = re.search(r'^(.*),\s*([a-z]{2})\s*([0-9]{5}).*$',
                ...
                properties = {
                    ...
                    'postcode': zipcode,
                    'country': 'US',
                    'lat': float(response.xpath('//a[contains(@class, "select-store-button")]/@data-store-latitude').extract_first()),
                    'lon': float(response.xpath('//a[contains(@class, "select-store-button")]/@data-store-longitude').extract_first()),
