...
class VakifBankTRSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        payload = {"CountryId":0,"WhichOne":2,"BranchNameOrCode":"","CityId":-1,"TownId":-1,"NeighborhoodId":-1,"Options":[]}
        yield JsonRequest(url=url, method="POST", body=json.dumps(payload), callback=self.parse)
        payload = {"CountryId":0,"WhichOne":1,"BranchNameOrCode":"","CityId":-1,"TownId":-1,"NeighborhoodId":-1,"Options":[]}
        yield JsonRequest(url=url, method="POST", body=json.dumps(payload), callback=self.parse)
