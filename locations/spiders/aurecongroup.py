...
class AureconGroupSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for location in response.xpath(".//h4"):
            addr = location.xpath(".//following-sibling::div")[0].xpath(
                ".//div/span/following-sibling::div"
            )[0]
            addr = " ".join(
                [
                    addr.xpath(".//span/text()")
                    .extract()[i]
                    .replace("\t", "")
                    .replace("\n", "")
                    .replace("\r", "")
            ...
            coordinates = str(
                location.xpath(
                    './/following-sibling::div//a[@target="_blank"]/@href'
                ).extract_first()
            )
            properties = {
                "ref": location.xpath(
                    './/following-sibling::div//span[@itemprop="telephone"]/text()'
                )
                .extract_first()
                .strip(),
                "brand": "Aurecon Group",
