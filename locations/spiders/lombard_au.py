    ...
    def parse(self, response: Response) -> Iterable[Feature]:
        ...
        for store in response.xpath("//table/tbody/tr"):
            if store.xpath('./td[@colspan="6"]'):
                if map_url := store.xpath('.//iframe[contains(@src, "https://www.google.com/maps/embed")]/@src').get():
                ...
                if branch := store.xpath("./td[1]//text()").get():
                    branch = branch.strip()
                    phone = store.xpath('./td/a[contains(@href, "tel:")]/@href').get()
