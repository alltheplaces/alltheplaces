    ...
    def parse(self, response: Response) -> Iterable[Feature]:
        ...
        for store in response.xpath('//table/tbody/tr'):
            if store.xpath('./td[@colspan="6"]'):
                lat, lon = url_to_coords(store.xpath('.//iframe[contains(@src, "https://www.google.com/maps/embed")]/@src').get())
                ...
                properties = {
                    ...
                    "addr_full": merge_address_lines(store.xpath('./td[contains(@class, "party-shop-finder-address")]//text()').getall()),
                ...
                if "Events By Lombard" in properties["branch"]:
                    properties = {}
                    continue
                hours_string = " ".join(filter(None, map(str.strip, store.xpath('./td[contains(@class, "party-shop-finder-trading-hours")]//text()').getall())))
