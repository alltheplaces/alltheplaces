...
class UPSFreightServiceCenter(scrapy.Spider):
    ...
    def parse_location(self, response):
        ...
        properties = {
            "addr_full": response.xpath(
                '//span[contains(@id, "Address")]/text()'
            ).extract()[0],
            "city": response.xpath('//span[contains(@id, "Zip")]/text()')
            .extract()[0]
            .split(",")[0],
            "state": response.xpath('//span[contains(@id, "Zip")]/text()')
            .extract()[0]
            .split(", ")[1]
            .split(" ")[0],
            "postcode": response.xpath('//span[contains(@id, "Zip")]/text()')
            .extract()[0]
            .split(", ")[1]
            .split(" ")[1],
            "ref": ref,
            ...
            "phone": response.xpath(
                '//span[contains(@id, "Telephone")]/text()'
            ).extract()[0],
            "name": response.xpath('//span[contains(@id, "lName")]/text()').extract()[
                0
            ],
    ...
    def parse_state(self, response):
        location_urls = response.xpath(
            '//*[@id="app_ctl00_scTable_hlDetail"]/@href'
        ).extract()
