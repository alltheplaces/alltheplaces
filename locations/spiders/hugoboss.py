...
from locations.categories import apply_clothes, Clothes
...
class HugoBossSpider(Spider):
    ...
    start_urls = ["https://api.hugoboss.eu/s/UK/dw/shop/v22_10/stores?client_id=871c988f-3549-4d76-b200-8e33df5b45ba&latitude=53.6912856662977&longitude=-2.0727839000000072&count=200&maxDistance=100000000&distanceUnit=mi&start=0"]
                ...
                for day_number, day_hours in opening_hours.items():
                    if type(day_hours[0]) is list:
                        for day_hours_period in day_hours:
                            item["opening_hours"].add_range(DAYS[int(day_number) - 1], day_hours_period[0], day_hours_period[1])
                    else:
