...
class FrysElectronisSpider(scrapy.Spider):
    ...
    start_urls = ("https://www.frys.com/ac/storeinfo/storelocator",)

    def parse_store(self, response):
        address_lines = response.xpath(
            '//div[@id="rightside"]/div[@id="text3"]/div[@id="address"]//b/text()'
        ).extract()
        address = ", ".join([a.strip() for a in address_lines])
        phone = [
            t
            for t in response.xpath(
                '//div[@id="rightside"]/div[@id="text3"]/div[@id="address"]//text()'
            ).extract()
            if "Phone" in t
        ]
        coordinates = [
            c
            for c in response.xpath(
                '//div[@id="rightside"]/div[@id="text3"]/div[@id="maps"]/text()'
            ).extract()
            if "°" in c
        ]
        ...
        if coordinates and len(coordinates) == 1:
            ...
            mapsLink = response.xpath(
                '//div[@id="rightside"]/div[@id="text3"]/div[@id="maps"]/a/@href'
            ).extract_first()
            if "ll=" in mapsLink:
