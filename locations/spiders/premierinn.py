...
class PremierInnSpider(scrapy.spiders.SitemapSpider):
    ...
    def parse_location(self, response):
        ...
        properties = {
            ...
            "name": response.xpath('//meta[@itemprop="name"]/@content').extract_first().strip(),
            "street_address": response.xpath('//span[@itemprop="streetAddress"]/text()').extract_first(),
            "city": " ".join(
                filter(
                    None,
                    response.xpath('//span[@itemprop="addressLocality"]/text()').extract(),
                )
            ),
            ...
            "image": response.urljoin(response.xpath('//a[@id="gallery-item-single"]/img/@src').extract_first()),
            "phone": response.xpath("//*[@hotel-phone-number]/@hotel-phone-number").extract_first(),
