...
class PremierInnSpider(scrapy.Spider):
    ...
    start_urls = (
        'https://www.premierinn.com/gb/en/hotels.html',
    )

    def parse(self, response):
        urls = response.xpath('//div[@class="pi-hotel-directory__county"]//a/@href').extract()

        for url in urls:
            yield scrapy.Request(response.urljoin(url), callback=self.parse_city)

    def parse_city(self, response):
        urls = response.xpath('//div[@class="seo-hotel-listings-wrapper"]//a/@href').extract()

        for url in urls:
            formatted_url = BASE_URL + url
            yield scrapy.Request(response.urljoin(formatted_url), callback=self.parse_location)
    ...
    def parse_location(self, response):
        ...
        try:
            properties = {
                ...
                response.xpath('//span[@itemprop="addressLocality"][1]/text()').extract_first() + ' ' +
                response.xpath('//span[@itemprop="addressLocality"][2]/text()').extract_first(),
