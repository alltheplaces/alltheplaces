...
class Spencers(scrapy.Spider):
    ...
    name = 'spencers'
    chain_name = "Spencer's"
    download_delay = 0.2
    allowed_domains = ('spencersonline.com', )
    start_urls = (
        'https://www.spencersonline.com/custserv/locate_store.cmd',
    )

    def parse(self, response):
        store_list = response.xpath('.//div[@class="body_wrap "]/div/script[3]/text()').extract_first()
        ...
        for store in store_properties:
            if 'var allStores = [];' in store:
                ...
                number = re.search(r"store.STORE_NUMBER = '(.*?)';", str(store)).group(1)
                name = re.search(r"store.ADDRESS_LINE_1 = '(.*?)';", str(store)).group(1)
                addr_full = re.search(r"store.ADDRESS_LINE_2 = '(.*?)';", str(store)).group(1)
                ...
                country = re.search(r"store.COUNTRY_CODE ='(.*?)';", str(store)).group(1)
