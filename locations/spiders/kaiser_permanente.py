...
class KaiserPermanenteSpider(scrapy.Spider):
    ...
    allowed_domains = ['healthy.kaiserpermanente.org', 'locations.kaiserpermanentejobs.org']
    start_urls = [
    ...
    def parse_location(self, response):
        data = json.loads(response.xpath(
            '//script[@type="application/json" and contains(text(), "address")]/text()').extract_first())
        ...
        properties = {
            ...
            'phone': response.xpath(
                'normalize-space(//div[@class="fb__phone-number styling-5-marketing"]/text())').extract_first(),
