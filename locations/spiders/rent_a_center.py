...
class RentACenterpider(scrapy.Spider):

    name = "rent_a_center"
    allowed_domains = ["locations.rentacenter.com"]
    download_delay = 0
    start_urls = (
        'https://locations.rentacenter.com/',
    )
    ...
    def parse_city_stores(self, response):
        stores = response.xpath('//div[@class="address-header"]/a/@href')
        print(stores)
        for store in stores:
    ...
    def parse(self, response):
        urls = response.xpath('//ul[@class="list-group"]/li/a/@href').extract()
        print(urls)
        for path in urls:
