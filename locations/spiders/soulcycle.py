...
class SoulCycleSpider(scrapy.Spider):
    ...
    def parse_store(self, response):
        ...
        properties = {
        'name': response.xpath('//span[@class="studio-name"]/@data-studio-name').extract_first(),
        'ref': response.xpath('//span[@class="studio-name"]/@data-studio-name').extract_first(),
        ...
        'lat': response.xpath('//div[@class]/div[@class]/script').extract()[-1].split('LatLng(')[1].split(')')[0].split(',')[0],
        'lon': response.xpath('//div[@class]/div[@class]/script').extract()[-1].split('LatLng(')[1].split(')')[0].split(',')[1],
        }
