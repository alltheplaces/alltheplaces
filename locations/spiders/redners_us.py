...
class RednersUSSpider(SitemapSpider):
    ...
    def parse(self, response):
        ...
        properties = {
            "ref": response.url,
            "name": response.xpath('//article[contains(@class, "ldm_map_location")]//h1[contains(@class, "entry-title")]/text()').get(),
            "lat": init_map_js.split("lat: ", 1)[1].split(",", 1)[0].strip(),
            "lon": init_map_js.split("lng: ", 1)[1].split("}", 1)[0].strip(),
            "addr_full": clean_address(response.xpath('//article[contains(@class, "ldm_map_location")]//div[contains(@class, "entry-content")]/p[position() <= 2]//text()').getall()),
            "phone": " ".join(response.xpath('//article[contains(@class, "ldm_map_location")]//div[contains(@class, "entry-content")]/p[last() - 2]/text()').getall()).strip(),
            "website": response.url,
            "opening_hours": OpeningHours(),
        }

        hours_string = "Mo-Su: " + " ".join(response.xpath('//article[contains(@class, "ldm_map_location")]//div[contains(@class, "entry-content")]/p[last() - 1]/text()').getall()).strip()
