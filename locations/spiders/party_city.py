...
class PartyCitySpider(scrapy.Spider):
    name = "party_city"
    item_attributes = { 'brand': "Party City" }
    allowed_domains = ['stores.partycity.com']
    start_urls = (
        'https://stores.partycity.com/sitemap.xml',
    )
    download_delay = 0.2
    ...
    def parse_store(self, response):
        ...
        if response.request.meta.get('redirect_urls'):
            ...
            script = json.loads(response.xpath('//script[@type="application/ld+json" and contains(text(), "streetAddress")]/text()').extract_first())
            data = script[0]
            ...
            properties = {
                ...
                'country': response.xpath('//body[contains(@class,"loading-search-result")]/@data-country-code').extract_first(),
