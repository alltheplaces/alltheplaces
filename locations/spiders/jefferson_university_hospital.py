...
class JeffersonUniversityHospitalSpider(Spider):
    ...
    async def start(self) -> AsyncIterator[Request]:
        ...
        yield Request(url=url, headers=headers, callback=self.get_number_locations)
    ...
    def get_number_locations(self, response):
        ...
        headers = {"Authorization": "Basic amgtcHJvZC1yZWFkb25seTpjMyoyZEpOdm1jYVRReTch"}
        yield Request(url=url, headers=headers, callback=self.get_details_location)

    def get_details_location(self, response):
        domain = "https://www.jeffersonhealth.org"
        for url in response.json().get("response", {}).get("docs", {}):
            yield Request(url=f'{domain}{url.get("page_url")}.model.json', callback=self.parse_location)
