...
class WINETIMEUASpider(CrawlSpider):
    ...
    rules = [
        Rule(
            LinkExtractor(allow=r"^https:\/\/winetime\.ua\/store\/[^/]+\/$", restrict_xpaths='//div[contains(@class, "city-choise")]/ul/li/a'),
            callback="parse",
        )
    ]
    ...
    def parse(self, response):
        for location in response.xpath('//div[contains(@class, "city_content")]'):
            ...
            properties = {
                ...
                "street_address": re.sub(r"Адреса\s+:\s+", "", location_details.xpath('./div[contains(text(), "Адреса")]/text()').get("")).strip(),
                "city": response.xpath('//h1[@class="title"]/text()').get("").strip(),
                "phone": re.sub(r"Телефон\s+:\s+", "", location_details.xpath('./div[contains(text(), "Телефон")]/text()').get("")).strip(),
                "website": response.url,
                "opening_hours": OpeningHours(),
            }
            location_key = properties["street_address"] + properties["city"]
            properties["ref"] = sha1(location_key.encode("UTF-8")).hexdigest()
            properties["opening_hours"].add_days_range(DAYS, *map(str.strip, re.sub(r"Час роботи\s+:\s+", "", location_details.xpath('./div[contains(text(), "Час роботи")]/text()').get("")).split("-", 1)), "%H:%M")
