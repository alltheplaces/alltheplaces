...
RAW_HEADERS = """Host: www.wyndhamhotels.com
...
def process_headers():
    return {k.split(": ")[0]:k.split(": ")[1] for k in RAW_HEADERS.splitlines()}
...
def create_url(brand, city, state, unique_url, tier_id):
    ...
    state_name = f"-{state.replace(' ','-').lower()}" if state.lower() != 'other than us/canada' else ''
    url += state_name + "/"
            ...
            for state in country['states']:
                ...
                for city in state['cities']:
                    ...
                    for property_ in city['propertyList']:
                        ...
                        url = create_url(brand_id, city_name, state_name, unique_url, brand_tier)
                        yield scrapy.Request(f"https://{self.allowed_domains[0]}/{url}", self.parse_property, meta={'id': property_id, 'country_code': country_code, "brand_name": brand_name},headers=process_headers())
