    ...
    def parse(self, response):
        for location in response.xpath('//div[@class="thumbnail"]'):
            properties = {
                ...
                "addr_full": list(filter(None, map(str.strip, location.xpath('./div[@class="caption"]/small[last()]//text()').getall())))[0],
                "phone": list(filter(None, map(str.strip, location.xpath('./div[@class="caption"]/small[last()]//text()').getall())))[1].split(",")[0].strip(),
            ...
            hours_text = list(filter(None, map(str.strip, location.xpath('./div[@class="caption"]/small[last()]//text()').getall())))[-1]
