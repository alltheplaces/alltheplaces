...
class SpecsaversNLSpider(CrawlSpider):
    ...
    rules = [
        Rule(
            LinkExtractor(allow=r"^https:\/\/www\.specsavers\.nl\/winkelzoeker\/(?!winkeloverzicht)((?!<\/)(.(?!-hoorzorg))+)$"),
    ...
    def parse_store(self, response):
        ...
        hours_raw = (" ".join(response.xpath('//tr[@itemprop="openingHours"]/@content').getall())).replace("Gesloten", "0:00 - 0:00").replace(" - ", " ").split()
