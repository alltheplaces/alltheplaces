...
class AppleSpider(scrapy.Spider):
            ...
            try:
                ...
                for day_range in day_ranges.split(","):
                    ...
                    for day in DAY_MAPPING[
                        DAY_MAPPING.index(start_day) : DAY_MAPPING.index(end_day)
                    ]:
    ...
    def parse_shops(self, response):
        ...
        properties.update(
            {
                ...
                "phone": store_details.xpath(
                    './/span[contains(@class,"store-phone")]/text()'
                ).extract_first(),
                "addr_full": store_details.xpath(
                    './/span[contains(@class,"store-street")]/text()'
                ).extract_first(),
                "city": store_details.xpath(
                    './/span[contains(@class,"store-locality")]/text()'
                ).extract_first(),
                "state": store_details.xpath(
                    './/span[contains(@class,"store-region")]/text()'
                ).extract_first(),
                "postcode": store_details.xpath(
                    './/span[contains(@class,"store-postal-code")]/text()'
                ).extract_first(),
            }
        )
        ...
        hours = self.store_hours(
            response.xpath(
                '//div[contains(@class,"store-hours-row")]/div/text()'
            ).extract()
        )
        if hours:
        ...
        if latlon_elem:
            lat, lon = re.search(
                r"&lat=(.+)&long=(.+)", latlon_elem.extract_first()
            ).groups()
