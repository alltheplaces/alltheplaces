...
from locations.hours import OpeningHours, DAYS_BG, sanitise_day, day_range
...
class T_Market_BGSpider(SitemapSpider):
    ...
    def parse(self, response):
        ...
        item['addr_full'] = response.xpath('//p[contains(text(), "Адрес:")]/following-sibling::p[1]/text()').get(default='').strip() if response.xpath('//p[contains(text(), "Адрес:")]') else None
        
        item['opening_hours'] = OpeningHours()
        # Some pages have split the opening days in multiple elements like <strong>Понеделник - <b>Неделя</b></strong>
        oh_days = response.xpath('//*[contains(text(), "Понеделник") or contains(text(), "Вторник") or contains(text(), "Сряда") or contains(text(), "Четвъртък") or contains(text(), "Петък") or contains(text(), "Събота") or contains(text(), "Неделя")]//text()').getall()
        self.log(oh_days)
        ...
        for index, days in enumerate(oh_days):
            if len(days)==2:
                item['opening_hours'].add_days_range(day_range(days[0], days[1]), oh_hours[index][0], oh_hours[index][1])
            else:
