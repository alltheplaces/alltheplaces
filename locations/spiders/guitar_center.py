...
day_mapping = {'MONDAY': 'Mo','TUESDAY': 'Tu','WEDNESDAY': 'We','THURSDAY': 'Th',
            'FRIDAY': 'Fr','SATURDAY': 'Sa','SUNDAY': 'Su'}

class GuitarCenterSpider(scrapy.Spider):
    name = "guitar_center"
    chain_name = "Guitar Center"
    ...
    def parse(self, response):
        ...
        store_counts = response.xpath('//span[@class="c-directory-list-content-item-count"]/text()').extract()
        store_counts = [int(re.search(r'(\d+)', store_count).group()) for store_count in store_counts]
        location_dir_items = dict(zip(urls,store_counts))

        is_store_list = response.xpath('//a[@class="c-location-grid-item-link"][1]/@href').extract()
    ...
    def parse_store(self, response):
        ...
        hours = self.parse_hours(response.xpath('//div[@class="c-location-hours-details-wrapper js-location-hours"]'))
