...
class FriarTuxUSSpider(Spider):
    ...
    def parse(self, response):
        for link in response.xpath('//a[contains(@href, "/locations-") and contains(@href, ".html")]'):
            ...
            directions = card.xpath('.//a[contains(@href, "daddr=")]/@href').get()
            coordinates = parse_qs(urlparse(directions).query).get("daddr", [""])[0].split(",")
            ...
            item = Feature(
                ...
                lat=coordinates[0],
                lon=coordinates[1],
                phone=card.xpath('.//a[starts-with(@href, "tel:")]/@href').get().removeprefix("tel:"),
                website=website,
            )
            ...
            for line in card.xpath(".//text()").getall():
                if closed_day := re.search(
                    ...
                    closed_days.append(closed_day.group(1))
                elif any(
                    day in line.lower()
            ...
            for day in closed_days:
                hours.add_range(day, "closed", "closed")
