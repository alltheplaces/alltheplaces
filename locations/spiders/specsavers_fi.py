...
class SpecsaversFISpider(CrawlSpider):
    ...
    def parse_store(self, response):
        ...
        hours_raw = (" ".join(response.xpath('//tr[@itemprop="openingHours"]/@content').getall())).replace("Suljettu", "0:00 - 0:00").replace(" - ", " ").split()
        hours_raw = [hours_raw[n : n + 3] for n in range(0, len(hours_raw), 3)]
        for day in hours_raw:
            if day[1] == "0:00" and day[2] == "0:00":
                continue
            properties["opening_hours"].add_range(DAYS_FI[day[0].title()], day[1].upper(), day[2].upper())
