...
class KiaGBSpider(Spider):
    ...
    start_urls = ["https://www.kia.com/api/kia_uk/findByDealer.list?displayYn=Y&delYn=N&pagePerLines=99999999&locale=undefined-undefined&usedSellYn=N&servicingYn=Y"]
    ...
    def parse(self, response):
        for location in response.json()["data"]["list"]:
            for location_type in ["sales", "service"]:
                ...
                hours_text = " ".join(["{}: {}".format(day_hours["name"], day_hours["hours"]) for day_hours in location.get(f"{location_type}Hours", [])])
