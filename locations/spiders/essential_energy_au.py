...
class EssentialEnergyAUSpider(RosettaAPRSpider):
    ...
    data_files = [
        RosettaAPRDataFile(url="./layers/Zone_Substations_Essential.geojson", file_type="geojson", encrypted=True, callback_function_name="parse_zone_substations"),
        RosettaAPRDataFile(url="./layers/Supply_Point_Station_Essential.geojson", file_type="geojson", encrypted=True, callback_function_name="parse_bulk_supply_points"),
    ]
    ...
    def parse_zone_substations(self, features: list[dict]) -> (list[dict], RosettaAPRDataFile):
        ...
        next_data_file = RosettaAPRDataFile(url="./essential_data/Zone_Substation_Data_Technical_Essential.csv", file_type="csv", encrypted=True, callback_function_name="parse_zone_substation_attribs", column_headings=["name", "voltages", "transformer_1_capacity_mva", "transformer_2_capacity_mva", "transformer_3_capacity_mva"])
        ...
        for properties in existing_features:
            ...
            if properties["name"] in features_dict.keys():
                ...
                if transformer_1_capacity_mva_str := features_dict[properties["name"]]["transformer_1_capacity_mva"]:
                    transformer_1_capacities = sorted(list(set(map(lambda x: float(x), re.findall(r"(\d+(?:\.\d+)?)", transformer_1_capacity_mva_str)))), reverse=True)
                ...
                if transformer_2_capacity_mva_str := features_dict[properties["name"]]["transformer_2_capacity_mva"]:
                    transformer_2_capacities = sorted(list(set(map(lambda x: float(x), re.findall(r"(\d+(?:\.\d+)?)", transformer_2_capacity_mva_str)))), reverse=True)
                ...
                if transformer_3_capacity_mva_str := features_dict[properties["name"]]["transformer_3_capacity_mva"]:
                    transformer_3_capacities = sorted(list(set(map(lambda x: float(x), re.findall(r"(\d+(?:\.\d+)?)", transformer_3_capacity_mva_str)))), reverse=True)
