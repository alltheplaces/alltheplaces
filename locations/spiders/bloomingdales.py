...
class BloomingdalesSpider(scrapy.Spider):
    name = 'bloomingdales'
    chain_name = "Bloomingdale's"
    allowed_domains = ['bloomingdales.com']
    download_delay = 0.2
    start_urls = (
        'https://locations.bloomingdales.com/',
    )
    ...
    def parse_hours(self, hours):
        ...
        for hour in hours:
            if hour["intervals"]:
                opening_hours.add_range(day=DAY_MAPPING[hour["day"]],
    ...
    def parse(self, response):
        urls = response.xpath('//li[@class="c-LocationGridList-item"]//a[contains(text(), "Website")]/@href').extract()
    ...
    def parse_store(self, response):
        ...
        hours = self.parse_hours(response.xpath('//span[contains(@class, "c-location-hours-today")]/@data-days').extract_first())
