...
class NextSpider(scrapy.Spider):
    ...
    def parse(self, response): #high-level list of states
        countries=response.xpath('//select[@id="country-store"]/option/@value').extract()
        for country in countries:
            req={"country":country}
            yield scrapy.FormRequest("http://stores.next.co.uk/index/stores",formdata=req,method="POST", callback=self.parse_city)

    def parse_city(self, response): #high-level list of cities
        cities=response.xpath('//option/@value')
        for city in cities:
            yield scrapy.Request('http://stores.next.co.uk/stores/single/'+city.extract(), callback=self.parse_shop)
            

    def parse_shop(self, response): #high-level list of states
        ...
        for place in range(1,len(parts)):
            ...
            try:
                hours=self.store_hours({'Mo':shop['mon_open']+':'+shop['mon_close'],
                'Tu':shop['tues_open']+':'+shop['tues_close'],'We':shop['weds_open']+':'+shop['weds_close'],'Th':shop['thurs_open']+':'+shop['thurs_close'],
                'Fr':shop['fri_open']+':'+shop['fri_close'],'Sa':shop['sat_open']+':'+shop['sat_close'],'Su':shop['sun_open']+':'+shop['sun_close'],
                })
