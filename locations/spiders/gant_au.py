    ...
    def parse(self, response: Response) -> Iterable[Feature]:
        for location in response.xpath('//div[contains(@class, "stores-data")]/div'):
            ...
            for hours_range in hours_ranges:
                if m1 := re.search(r"((?:(?:MON|TUE|WED|THU|FRI|SAT|SUN)\s*-\s*(?:MON|TUE|WED|THU|FRI|SAT|SUN)|(?:MON|TUE|WED|THU|FRI|SAT|SUN)))", hours_range.upper()):
                    if m2 := re.search(r"((?:CLOSED|\d{1,2}(?:[:\.]\d{2})?\s*[AP]M\s*-\s*\d{1,2}(?:[:\.]\d{2})?\s*[AP]M))", hours_range.upper()):
