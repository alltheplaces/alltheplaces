...
def extract_geo(item, src):
    if src:
        set_geo(item, get_key(src, lat_keys), get_key(src, lon_keys))
    return item


def set_geo(item, lat, lon):
    if lat and lon:
        try:
            lat_val = float(lat)
            lon_val = float(lon)
...
def has_geo(item):
    return item.get('lat') and item.get('lon')


def extract_html_meta_details(item, response):
    """
...
def extract_ldjson(brand, response, required_type):
...
def parse_ldjson(brand, entry_or_entries, required_type, response=None):
    ...
    for entry in ensure_list(entry_or_entries):
        ...
        source_data(item)['ld_json'] = entry
