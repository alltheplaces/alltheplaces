...
class Command(scrapy.commands.genspider.Command):
    ...
    def _generate_template_variables(
        ...
        tvars = {
            "project_name": self.settings.get("BOT_NAME"),  # ty: ignore[unresolved-attribute]
            "ProjectName": string_camelcase(self.settings.get("BOT_NAME")),  # ty: ignore[unresolved-attribute]
            "module": module,
            "name": name,
            "url": url,
    ...
    def _genspider(
        ...
        if self.settings.get("NEWSPIDER_MODULE"):  # ty: ignore[unresolved-attribute]
