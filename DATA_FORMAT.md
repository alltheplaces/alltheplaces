...
| `addr:unit`           | The sub-address part of the address. Examples include "Unit 12" or "Suite 320".
| `addr:street`         | The street name part of the address, including any street type applicable (such as "Road" or "Street").
| `addr:street_address` | The street address. Typically used when it's not practical to reliably split apart properties such as `addr:unit`, `addr:floor`, `addr:housenumber`, `addr:street`.
...
Some All the Places spiders may model more precise and/or country-specific address information as [extra properties](#extras) if the source data provides a precise model of address information. Examples of such properties are `addr:floor`, `addr:hamlet` and `addr:suburb`.
