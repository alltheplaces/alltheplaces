...
- **Known false-positive checks — don't treat these as blocking on their own:**
  - `ci/check_spider_naming_consistency.py` failing because a brand name legitimately ends in something that reads like a country-code suffix (e.g. "Basilic and Co"). The fix is `skip_auto_cc_spider_name = True` on the spider class — a trivial, non-controversial fixup, not a real bug (davidhicks, PR #17950).
